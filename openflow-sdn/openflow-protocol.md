In this article, we will get to know the basics of OpenFlow protocol which is the standard protocol in the traditional software defined networks. This article will go through the basics of software defined networking(SDN) and OpenFLow(OF), the working of OF protocol and some terminologies, advantages over traditional networking architecture and conclude with alternatives to OpenFlow that can be used in SDN.  

As a prerequisite to the article, the reader should be aware of tradtional networks, OSI model and related technolgies like switching, routing, etc. Having an understanding of network automation, data plane and control plane is also a good to have. 

### Table of Content
1. What is OpenFlow Protocol?
2. Background on traditional SDN
3. Basics of OpenFlow
4. Advantages of OpenFlow
5. Alternatives and other means to SDN
...
Before moving further, the term "switch" denotes any network device capable of using OF protocol, and not the Layer 2 device. In the layer 2 of the [OSI model](https://en.wikipedia.org/wiki/OSI_model), layer 2 refers to the data layer where the traitional switches reside. They are responsible for forwarding **ethernet 'frames'** on the wire. There are also several features like VLANS, STP and port aggregation that can be implemented at layer 2. 
...
3. Most current devices support OpenFlow, it is not enabled by default, but we can easily enable and use it in the transition to SDN. 
...
1. **REST APIs:** REST APIs are the most popular choice after OpenFlow to communicate with the switches. All the controller applications also have a REST module supported with them. The user can query the switch database to POST or GET data. "Curl" is one of the most popular tools for this. An example of a REST call on the RYU controller to get all the switches in your network would be
