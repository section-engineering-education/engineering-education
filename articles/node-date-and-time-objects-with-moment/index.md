...
```
Moment also run on the browsers. It creates a global moment object, which is used to access date with `<script>` tag.
Moment has gained good reputation for being able to deliver its objective. I this article we shall look at
-   date formating
-   date parsing
-   date manupulation
-   date validation
...
moment().add(7, 'days');
```
Moment supports abbreciations such as years(y), months(M), weeks(w), days(d), minutes(M), etc.
```js
moment().add(7, 'y');
moment().add(7, 'M');
moment().add(7, 'd');
...
```js
 ...
 ```
 This example will return moment object date; you can format the date to human readable date
...
```
However, when dealing with partial dates, there are possibilities to run into unexpected results such as shown in the example below.
...
```
This example will return true yet the input provided is not a valid date. This is because the string has "2020" which will be interpreted as "YYYY". To avoid this, you need to use strict parsing mode with "true" as the third argument.
```js
const moment = require('moment');
console.log(moment(
...
```
By default moment, return duration in milliseconds, however you can pass your favourite unit of measurement as the second argument
```js
var moment = require('moment');
...
Ex1 check if a moment [is before](https://momentjs.com/docs/#/query/is-before/) another moments.
...
```
Ex2 check if a moment [is same to](https://momentjs.com/docs/#/query/is-same/) another moment.
```js
var moment = require('moment');
console.log(moment('2020-08-12').isSame('2020-08-12'));
console.log(moment('2020-08-12').isSame('2020-07-11'));
```
Ex3 check if a moment [is between](https://momentjs.com/docs/#/query/is-between/) another moments.
```js
var moment = require('moment');
...
```
Moment time zones are bundled with [webpacks](https://momentjscom.readthedocs.io/en/latest/moment-timezone/00-use-it/04-webpack/) to remove unwanted data and remain only with zone and date range data. [`moment-timezone-data-webpack-plugin`](https://momentjs.com/docs/#/use-it/webpack/) helps to buddle timezone data to minimal possible size to round 900 KBs and can be significantly reduced to between [1MB to 35KBs minified plus gzipped](https://github.com/gilmoreorless/moment-timezone-data-webpack-plugin).
```js
// webpack.config.js
...
1. `moment().tz(String)` converts date to provided timezone.
...
Moment is mutable while date-fns is immutable. Moment keeps changing its own sate while date-fns always returns a new Date instance.
