...
status: publish
published: true
url: /engineering-education/nodejs-adminbro/
title: How To Auto-generate Admin Panels for Node.js with AdminBro
description: This article guides on how to create powerful Admin Panels in Node.js using AdminBro. Admin Panels allow you to manage data more effectively.
author: kevin-kimani
...
<!--more-->
[AdminBro](https://adminbro.com/) is an open-source Node.js package that adds an auto-generated admin dashboard where we can manage data in a single panel in our applications. This adds the ability to find, monitor, and update data during development. Some AdminBro's features include form validation, customization, and access control. 

### Goal
In this tutorial, we will set up a working admin interface that can be used to start managing data in a Node.js application.

### Prerequisites
To follow along, you will need to:

1. Have a basic knowledge of [JavaScript](https://www.w3schools.com/js/DEFAULT.asp).
...
First, create a folder and name `Express-AdminBro` then open it on your IDE, I will be using VS Code. Inside this folder, setup a new project using the `npm init -y` command to generate an initial `package.json` file for our project. We need several npm packages from the npm registry. They include:

- express: Express is a Node.js framework for building web applications and backend APIs.

- mongoose: Mongoose is an Object Data Mapper for MongoDB that connects the application to the database.
...
- tslib: `tslib` is a module runtime library for TypeScript with various helper functions. It is quite useful when dealing with AdminBro.
...
Inside our `Express-AdminBro` application directory, add another folder named `config` and inside it, create a `db.config.js` file. Below is the code that establishes the connection to the database using the `mongoose` package:
...
`Mongoose` is a package that manages relationships between data and provides schema validation. When using mongoose, models are defined using a `Schema` interface. 
...
In the above code snippet, we have created a schema interface by importing the mongoose module. The `mongoose.Schema` method is instantiated to create the `AdminSchema`. This object takes the values that our MongoDB document will store. The values include:
...
At this point, we have the admin interface set up in our application. To ensure that everything is working, make sure that the database and server are up and running. We can now open the browser and navigate to `http:localhost:8000/admin` to see the admin dashboard:
...
### Conclusion
Admin Panels are great for increasing developer productivity, especially in handling database Queries.  However, the Node.js framework does not come bundled with one. If you are a web developer coming from Python/Django background which has a great Admin Panel integrated, this can be a little disappointing. By adding AdminBro to our Node.js applications, we enjoy an amazing set of features, extensibility, and various customization options. I hope you find this tutorial helpful. 
