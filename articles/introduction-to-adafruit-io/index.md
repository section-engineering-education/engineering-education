...
Cloud computing is the means of storing and accessing data over the Internet instead of using them locally. It allows sharing of resources across devices through centralized storage.
...
### Adafruit IO
Having understood a little about the Internet of Things and Cloud computing, let's understand what [Adafruit IO](https://io.adafruit.com) is about, and how it works.
...
It is highly recommended to go through the [previous article](/engineering-education/getting-started-with-nodemcu/), before continuing to read.

#### Objective
In our previous article, we worked with NodeMCU for alternative blinking of built-in LEDs. In this article, to demonstrate the working of Adafruit IO, we send (publish) the LED brightness readings to the Adafruit IO cloud via Arduino IDE (written in C), and receive (subscribe) them via a Python server.
...
- Similarly, make sure to go through the previous article for setting up the existing development environment

#### Implementation - Publisher module
In Arduino IDE, we have to import the necessary library for publishing the values. To import, use `#include <Adafruit_MQTT_Client.h>`.

Also, we have to define certain constants for initializing the server.
...
In the above snippet, we subscribe the data from the cloud, and set the brightness of LED by assigning the value to `led.value`. 
...
Thus, we published the values from a client module to the Adafruit IO cloud, and subscribed those values to another client, and displayed the LEDs based on the brightness levels.
...
### Conclusion
To conclude, we have learned about IoT deployments on the cloud using Adafruit IO. We also built a simple project for demonstration.
...
- We learned about a cloud provider called Adafruit IO.
