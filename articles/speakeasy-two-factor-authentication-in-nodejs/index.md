...
### Introduction
Two factor authentication is a security measure implemented in an application to increase security by providing two separate eveidences of authorization to the system. Two factor authentication(2FA) works beyond the username/email and password authentication. On of the ways through which 2FA is implemented is using the speakeasy library.
The Speakeasy library provides two-factor authentication using a one-time pin(OTP). The library provides an additional layer of security to an application's standards authentication process. Using the OTP, Speakeasy provides extra data required for account access. You can find the the download link and more about the library [here](https://github.com/speakeasyjs/speakeasy).
