...
First, launch Android Studio and create an `Empty Activity` project. All through this tutorial, we'll lay our focus on `activity_main.xml` file. This is where we'll declare and design the layout.
...
A baseline is a horizontal invisible line that is centred in a view. A view can only have one baseline used to align it horizontally.
...
Whenever we drag view `A`, `B` is dragged as well.

#### E). Circular position alignment

Constraint layout has the capability of aligning views with respect to the centre of another view at a given angle and radius. See the figure below.
...
*`layout_constraintCircle`* - the referenced view's id. Its centre serves as the centre for the alignment.

*`layout_constraintCircleAngle`* - the angle of displacement given in degrees ranging from `0 to 360`

*`layout_constraintCircleRadius`* - distance (in `DP`) between the centres of the two views. This determines how far a referencing view is placed from the referenced one.
...
Chains are one of the most awesome features of constraint layout. They simplify the alignment of views and help them utilize the space available effectively as desired. There are four chain styles, `compressed`, `spread`, `weighted`, and `packed` style.
...
So far we've learned about different techniques of constraining views where we do most of the alignment work. Constraint layout has design features called `Helpers` that enable us to align views with less code effectively as possible. They also make the whole design process faster and relatively maintainable.
...
A group is an assembly of views. If you want to work with a variety of views in the same way simultaneously, a group is a nice feature to use. To create a group, we need to refer to the ids of the views we want to be members of the group.
...
*`iii). aligned`* - aligns views in rows and columns.
