...
Then the last thing we would do is work on our `timer` so that it doesn’t show up next to the text area but on top.  (The timer won’t count up yet until we add some Javascript)
...
Then we call the `startTimer` in our `renderNewQuote`. Whenever a new quote is rendered we want our `timer` to restart. So inside this timer what we would do is,   for every second that we want our timer to be updated, `setInterval` will be used
