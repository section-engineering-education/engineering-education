...
date: 2020-10-30T00:00:00-16:00
topics: [Languages]
...
Now after having a brief about **web scraping** let's talk about the most important thing, that is "**legal issues**" surrounding the topic.

### How to know if the website is allowing web scraping?
- You have to add "/robots.txt" to the URL, such as www.facebook.com/robots.txt, so that you can see the scraping rules and see what is forbidden to scrap.
...
In www.facebook.com/robots.txt you can find this rule listed above, it means that a Discord bot has the permission to do **web scraping** for Facebook videos.
...
If the result is **200** then you have the permission to do **web scraping** to the website, but you also have to take a look at the scraping rules.
...
- You can scrape just the data that is available for the public like the prices a product, you can not scrape anything private, like a **Sign In** page.
...
As we know, **Python** have different libraries for different purposes.
...
You are ready to go, let's get started and learn a bit more on **web scraping** through two real-life projects.

### Reddit Web Scraper
One year ago, I wanted to build a smart **AI** bot, my aim was to make it talk like a human, but I had a problem, I didn't have a good dataset to train my bot on, so I decided to use **REDDIT** posts and comments.
...
After doing the process above on the first title on the page, you can see the following code with a highlight for the tag which holds the data you right-clicked on:
