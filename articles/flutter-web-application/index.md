...
Flutter for Web has recently moved from beta to stable. This is good news to developers since they can build highly secure and appealing websites. Flutter widgets are based on Google's material design and principles. This means that even inexperienced Flutter developers can design quality applications. 

### Goal
This tutorial guides you on how to create a simple Flutter web application.

### Prerequisites
- Some knowledge in Dart.
...
We use the `flutter create webapp` command to build a new project. This command allows the required dependencies to be downloaded and included in the application. You can replace `webapp` with your desired project name.
...
The above code will launch the application in the Chrome browser. Note, that `-d chrome` portion of the `run` command is only used when there are several connected devices.
...
In this step, we will look at the different folders and files included in the project. Open the web app in Visual Studio Code.  You will notice these folders; dart_tool, .idea, android, build, ios, lib, test, and web.  We will mainly be working with the lib and web folders. All our programming files should be included in the lib. These files are then compiled depending on the platform that one is targeting. In our case, the files will be compiled and stored in the web folder. The `index.html` file in the `web` folder will serve as an entry point to our application.
...
Create a new file in the `lib` folder and name it as `homepage.dart`. Next, insert a stateful widget in the `homepage.dart` file, as shown below. 
