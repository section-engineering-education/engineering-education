...
There are two unary operators for increasing and decreasing scalar values. They can be used as either before the variable (prefix: ++n) or after the variable (postfix: n++). In prefix mode, it increments (adds 1) first then uses the value, while in postfix it uses the value first and then increments. The same applies to the decrement (subtracts 1) operator.
