...
2. **`keypress`:** This event is fired when an alphabetic, numeric, or punctuation key is pressed down. Some browsers no longer support this event. Refer to [Mozilla documentation](https://developer.mozilla.org/en-US/docs/Web/API/Document/keypress_event#browser_compatibility) for `keypress` event browser compatibility details.
...
The complete source code for the above demo is available on [Github](https://github.com/Tsanguu/Javascript-Keyboard-Events). The control key is used in combination with other keys. Therefore, we need to note when the Control key is combined with different keys. The code below creates an alert when any key is pressed down (`keydown` event) except for the Control key.
...
Create two functions (updateYPosition and updateXPosition) for updating the object position. The function gives the object's updated position given the distance the object is moved along either axis.
```JavaScript
...
The `addEventListener()` method is used to attach an event handler to the object. The event handler listens for keydown events. Once the relevant keys are pressed, the `refreshPosition()` method is called to draw the object at its new position.
```JavaScript
