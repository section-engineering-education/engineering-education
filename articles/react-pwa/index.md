...
delete the `logo. svg` and remove everything in both `App.css` and `App.js` files before we start to build the application.
...
1. The TodoList component
2. The Form component
3. The Todo component
4. The App component

### The App component 
First, I have included the `App` component as is the parent component of the `TodoList` and the `Form` components. I have included a CSS file for our styling named `App.css` which is in the same `src` root folder as our *App.js* file.
...
```bash
    ...
    robots.txt
```
I will use these default icons from our create-react-app.
...
Our app is now installable by clicking the plus(+) icon on google chrome.
