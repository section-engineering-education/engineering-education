...
Whenever we submit a form on the client-side of any website, all the data of the form goes to the server-side. Usually, **form-data gets encoded before submitting it to the server**. We can do this by specifying the **enctype attribute in the \<form> tag in HTML.** If we don't specify it, form-data gets encode with the default type. This is usually the case when we are dealing with text-only data like name, email, password, etc.
...
Now, **Multer is an npm package that makes it easy to handle the file uploads.** It does it very efficiently, thus it is quite popular.
...
Next, **create a new HTML file and name it index.html and in that, we will create a form in it for uploading files.** Your HTML code should look something like this.
...
**Before moving forward, make sure that Node.js is installed in your system. To check, if it is installed, write this command in your terminal.**
...
Since our static site is ready so now **we can start initiating this project with npm.** To do this:-
...
I hope you are familiar with this command. In case you aren't, **this command creates a new package.json file. This file will help us to manage all the dependencies that we will install later on in this tutorial.**
...
Also, **most of the things that we'll be doing now will only help us only to set-up our project such that we can start using Multer. So, I am not going to explain each thing in detail because this is not the exact focus of this tutorial.** If you have a good understanding of Node.JS, express, and MongoDB, you will be able to understand it. **I will also try my best to write this part concise and clear so that you can follow along pretty easily.**

Now, before that, we have to install the **three most important npm packages that we need to continue this tutorial. These are:- express, body-parser and mongoose.** Thus, write this command in terminal:-
...
So, to do that i.e, **to make a MongoDB cluster in Atlas (cloud database) and make the connection with our project, please refer to these two videos.**

1. [This](https://www.youtube.com/watch?v=rPqRyYJmx2g) video will help you create a cluster.
2. [This](https://www.youtube.com/watch?v=Qn0SOL8vK8w) video will help you connect it to our project.
...
:rocket: **Don't forget to enter your connection URI string in the DB variable.** :rocket:
...
**If you have done everything right, then you should see this message on the terminal.**
...
**Before writing any code in the app.js file, we have to make some new folders and change the locations of some files.**
You must be wondering why we are putting so much effort into emphasizing these things. **It is because writing clean and manageable code with an organized folder structure is as important as writing the "right" code.** These kinds of folder structure and refactoring will help you with your bigger future projects also.
...
2. Create a second new folder and name it **views**. **This will hold our HTML file which we created at the beginning.**
...
**2)** Include the path package at the top. It is the build-in node.js package.

```javascript
const path = require("path");
```

**3)** Write the configuration code for setting up the ejs template engine and defining the path.
...
**4)** Now, we will create our first API endpoint, which is to render the HTML file that we build at the starting of this tutorial.
...
Now, **if you will restart your server** with the same command as earlier i.e,

```
nodemon server.js
```

and, if you do not run into any errors, **you should see the rendered HTML file you created earlier at the localhost:3000.**
...
1. **The actual files are never stored in the database. They are always stored someplace on the server.** In our tutorial, we will store the uploaded files in the **public folder.** This is because **all the files that are in the public folder are meant to be available to the public at the frontend.**
   Also, later in the tutorial, **we will also learn how to view those files on the front-end. So, that is another reason to store them in the public folder.**
...
**So, the next thing we need to move forward is to create a schema for storing the name of our uploaded files.**
We will do this with the help of the **mongoose package that we installed earlier.** To do this, follow these steps:
...
As I already told you earlier. **Multer is a node.js middleware for handling multipart/form-data, which is primarily used for uploading files.** For those who don't know what is a **middleware in node.js**:-
**It is a function that receives the request and response object when a user from the client-side makes any request.**
...
**We can add as many as middleware in this request-response cycle.**

**Let's start by installing the Multer. Write this command in your terminal.**
...
**Note:-** Make sure that the API to render the page should be at the end of all the API endpoints.
...
Since **we are going to store the uploaded files in our disk storage inside the files folder that we just created.** Thus, **let's start by defining the destination.** **So, copy the following code in your app.js file just below you wrote the code for configuration of static files**,and then I'll explain it.
...
In this code, **we are calling the multer function.** It takes certain options as arguments. So, **we will pass the dest (destination) option as an argument** and the **value of dest will be :- 'public/files'.**

After that, **we have to use this upload variable as the middleware** in the above-created API endpoint. So, **change that API endpoint like this:-**
...
Finally, we did **console.log(req.file) to see the details of the file that we are uploading.** This will help us to configure the multer in a more advanced way. Also, we will be able to do the filtering on those files.
...
**Finally, you can upload a file from your rendered page. When you will hit submit, you should see something like this on your terminal window.**

:rocket: This is my output, yours can be different based on what you uploaded. :rocket:
...
So, **with the information we just got, we will configure the multer in a more complex way** such that our files can become readable.
...
So, to do this, **replace the previous one-liner code for configuration with this code.**
...
We have completed the first step of configuring the multer. **Next, we will make a filter to filter out different kinds of files.** I will make a filter to upload only pdf files. You can make your kind of filter by referring to the code below.
...
Now, this piece of code is very simple. **Multer filter is just a function that also has req, file, and a callback function as its arguments.** In this, **we will check if the uploaded files are PDFs, if that so we will pass true in the callback function. If it isn't a PDF we will pass false along with an error in the callback function.**
