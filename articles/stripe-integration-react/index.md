...
To resolve the error, you must enter your public API key. We have discussed about entering the public key which is available on the dashboard. Enter the `PUBLISHABLE_KEY` in the `stripeKey` paramter under `CheckoutWithStripe.js` component. 
...
2. Load the stripe module. Install the stripe module at the backend using the command
   ...
   Once installed, load the module using the below code:
    ...
    ```
3. Charge the card by making an API call with the token information, amount to be charged, to the [Stripe charges API](https://stripe.com/docs/api/charges/create). The source parameter takes in token generated at the frontend as the input. For reference, I have mentioned it as `${data.name}`. 
