...
- [Table of contents](#table-of-contents)
    ...
    - [Vlan routing terminology](#vlan-routing-terminology)
...
In this method, multiple router interfaces are used, each connecting to a switch port in different VLANs. These interfaces are served as default gateways,and this required additional cabling when the network was to be expanded. Hence becoming more costly for the network cables and infrastructure.

#### Router-on-a-stick
In this method, unlike the legacy routing, one physical interface port is used for routing the traffic between the network segments. The network administrator need not create separate VLAN interfaces like `fa0/1` to `fa0/10`. Instead, all the interfaces from 1 to 10 are created with a single interface. This method is simple to implement and use for smaller to medium-sized organisation networks.
