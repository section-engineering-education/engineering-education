...
Adapting models like BERT to a specific task requires fine tuning, where you use a large dataset to train the models. This can be a tedious process.
You can give a GPT-3 model a language translation task, with the fewest examples and it will generate output.

You can instruct it to generate code and it does so with few examples. This is a very exciting characteristic of this language model. Although you might be wondering how it manages to deliver with little to no examples. The answer: few shot learning.
...
**Zero shot learning**. We can say this is training a model to do tasks it wasn’t explicitly trained to. The task is only described to the model, without examples. Consider the above example describing asking a model to translate a word from English to Swahili. The description could be “translate from English to Swahili”. The model understands natural language. Without examples, it takes on the translation task. Once more, no fine tuning is required.

GPT-3 is a few-shot learner. It can be a one-shot learner or zero-shot learner as well. For a deeper understanding of how GPT-3 works, check out this [post]( https://jalammar.github.io/how-gpt3-works-visualizations-animations/)
...
It would have a greater range of responses to queries. It could even crack jokes where appropriate. Recommendations to customers could be more detailed and of a higher quality. The list is endless. It could provide that almost-human touch to any interaction.

Vast language tasks without training data. As mentioned before, GPT-3 outperforms all other models when it comes to performing of specific tasks without any fine tuning. Fine tuning involves use of training data to train a model.

Considering adapting other models like BERT to a specific task requires fine tuning, it is a very attractive prospect to machine learning practitioners. Fine tuning of models is often a complex and inefficient process. Taking that step away from the process and ensuring that the output will still fulfill the desired task is every engineer’s dream.
