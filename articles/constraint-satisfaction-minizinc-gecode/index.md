...
description: Constraint-Satisfaction Problems (CSPs) represent problems in terms of variables, their possible values, and constraints on those values. In this tutorial, we will use MiniZinc, a programming language built for CSPs, and Gecode, a CSP solver, to model and solve a simple map-coloring problem.
...
A domain is, itself, a set of (a finite number of) possible values for a corresponding variable. So if *X={x<sub>1</sub>, x<sub>2</sub>, x<sub>2</sub>}* is your set of variables, then *D*={*d<sub>1</sub>*, *d<sub>2</sub>*, *d<sub>3</sub>*} is your set of domains, where the *i*th variable *x<sub>i</sub>* can be a value from the set *d<sub>i</sub>*.
...
You can convert the information about the graph into a Constraint-Satisfaction Problem. The variables could be the order in which you visit each city (where *x<sub>i</sub>* is the *i*th city visited). The domains can be the cities. The constraints would include "lower the cost of the path traveled on as much as possible", and "you can't visit the same city twice", all rewritten in mathematical notation.
...
Navigate to the [MiniZinc downloads page](https://www.minizinc.org/software.html) and click on the recommended installed that matches best with your operating system. If you don't see your OS, or need a 32-bit version, look for an alternative release [here](https://www.minizinc.org/ide/) or [here](https://github.com/MiniZinc/MiniZincIDE/releases/). In this tutorial, we use MiniZinc 2.5.3 on Windows 10.

![IMG1](/engineering-education/constraint-satisfaction-minizinc-gecode/img1.jpg)

Once you've downloaded the installer (named something like `MiniZincIDE-2.5.3-bundled-setup-win64.exe` in your Downloads folder), click to run it. If you're on Windows, and got a "Windows protected your PC" popup, click "More info" and then "Run Anyway". Follow the installer's prompts (the defaults are fine, accept the agreement) and click "Install" and then "Finish".
...
In the mathematics of CSPs, the [four-color theorem](https://nrich.maths.org/6291) tells us we can color *any* map with no more than 4 different colors. We'll solve our map-coloring problem for 4 colors, and later see if we can do so with fewer colors.

Our code is partly based on a simpler map-coloring example in the [MiniZinc documentation](https://www.minizinc.org/doc-2.5.3/en/modelling.html), Listing 2.2.2 in [other MiniZinc documentation](https://www.minizinc.org/doc-2.4.3/en/modelling2.html#arrays-and-sets), and a basic traveling-salesman example from the [University of Glasgow](http://www.dcs.gla.ac.uk/~pat/cpM/miniZinc.html). These can be investigated further by curious readers.

For now, we put a MiniZinc "solve" command at the end of our code, to make it look like this:
...
The "satisfy" keyword tells MiniZinc that we just need *some* kind of solution to the problem, not necessarily the "best".
...
In MiniZinc, a variable is defined like `var domain: identifier;`. The domain is the range of values that the variable (named by the identifier) can take on. In our map-coloring, we can define Texas as `var 1..num_colors: TX;`, which says "TX is a variable that can have a value from 1 to num_colors, inclusive".
...
If we had continued our earlier practice of using lone variables, the constraints for Texas would look something like this:
...
Finally, we want to see our solution in full. We'll use the "output" command, which can iterate through the States enum and print out the state abbreviations, colors, and formatting. At the very end of your code, add this line:
