...
We will be creating an Html index.php file in our root folder (CRUD folder), then include the Axios package in the head section of the Html using CDN.
...
We created four (4) methods in the code:
1. allRecords – this will send a POST request to fetch records where the pass request is 1. On successful, callback assign response.data to the app.users.
2. addRecord – this will send a POST request to add a new record where the pass request is 2. On successful callback Empty the data values and call allRecords methods.
3. updateRecord – this method will read data from products according to index and send a POST request to update the record where pass request: 3.
4. deleteRecord – this will send a POST request to delete the record where pass request: 4. On successful callback remove an index from products using splice().

Also, we defined the created option to call the allRecords() method after the instance is been created.

Following the steps above, we will be able to connect our Vue project with the MySQL database using PHP.