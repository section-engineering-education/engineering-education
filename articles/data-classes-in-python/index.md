...
We've created a method, `studentinfo()` which returns a formatted name and gpa as a string. We define a method in a dataclass like any other. We then change the `name` attribute of `student1` and his `gpa`. We then print the student information of the first student using the `studentinfo()` method. We can see the output changes. 
