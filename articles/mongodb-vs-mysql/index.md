...
status: publish
published: true
url: /engineering-education/mongodb-vs-mysql/
title: MongoDB and MySQL Comparison
description: After reading this article, you should know  
...
images:
  ...
  - url: /engineering-education/mongodb-vs-mysql/hero.jpg
    alt:  Image

---
A few years ago, applications had just a handful number of users. Today, the number of people using applications has immensely grown, which calls for more developed databases to support thousands of users. This article will compare the difference between relational and non-relational databases, and specifically MySQL and MongoDB.
<!-- more -->
It will highlight the advantages and disadvantages of using the two to help you choose the best depending on the project situation.
...
[Non-relational databases or NoSQL](https://www.mongodb.com/nosql-explained) (Not only SQL databases), unlike MySQL, stores data in a non-tabular format. NoSQL can handle unstructured data such as e-mails, documents, multimedia, and social media more efficiently. They are best to use whenever one has a large volume of data. 
...
### MongoDB
MongoDB is a document database with the scalability and flexibility that supports querying and indexing as per users' needs. It is a popular document-oriented and NoSQL database. The first MongoDB version was released in the year 2010. 

MongoDB does not require a fixed structure and is flexible enough to change in its schema-free implementation. Meaning it does not require a logical configuration of database parts. It is highly scalable and available. MongoDB is highly efficient and reliable. It is suitable for storage capacity and speed demands.

#### BSON Files
Documents in MongoDB are created and stored in BSON files (BSON stands for Binary JSON). A BSON file is a modified version of [JSON](https://www.json.org/json-en.html) (JavaScript Object Notation) files and supports all JavaScript files. JSON is a lightweight data-interchange format, easy for humans to read and write and for machines to parse and generate. 

JSON was derived from JavaScript, though it is independent of the language. BSON is preferable to JSON because it is easier to study, faster, and requires less storage. 

### MySQL
MySQL is an open-source and a relational database management system. The concept behind MySQL is that data is stored in rows and tables, then classified into a database. MySQL uses SQL language to access and manipulate the database. 

SQL does the following in a database:
1. Enables query execution.
2. It enables data retrieval.
3. Allows inserting, updating, and deleting of records.
4. Allows a user to create new databases.
5. Allows a user to create new tables and views.

Each table in MySQL stores separates information. The concept of MySQL [Joins](https://www.techopedia.com/definition/1213/join#:~:text=A%20join%20is%20an%20SQL,a%20relationship%20between%20the%20tables.&text=The%20type%20of%20join%20a,which%20records%20the%20query%20selects) is applied. The Join operations concept is a method of inter-linking data in one or more tables based on a standard column's values between the tables. Joins minimizes data duplication chances across the tables. 

Examples of platforms that are run by MySQL include Linux UNIX and Windows. Examples of web-based applications that use MySQL include Facebook, Twitter, and YouTube.
...
- MongoDB's schema is flexible. It can be modified to fit the user's needs. One can input data in whatever data model without any predefined schema. Also, the user is free to change data format or model any time without any hurdle.
- It is scalable. Using replica sets in MongoDB is possible. To increase the data load, one needs to add more server hardware power and more machine resources.  
- It supports expansion in the sense that it allows the addition of machines and RAM to the system. It is data-intensive, stores a lot of information, and queries a lot of data.
- It also allows for document validation and integrated systems.

#### Disadvantages of using MongoDB
- It uses extensive memory, and it may have massive data, some useless over time.
- It limits the document size to 16MB. Hence it requires more procedures for one to store a large file with more than 16MB.
- In some instances, it is comparatively slow compared to NoSQL, which leads to poor page response.
...
- It may take a lot of time and effort to perform some tasks, even for tasks requiring an automatic update, such as incremental back-ups.
- It is not easily scalable. It is not possible to use replica sets in MySQL. In some exceptional instances, scaling in MySQL is done using a robust server or even a complex sharding (breaking large tables into smaller ones across the multiple servers) solution is done. Powerful servers are not only costly but also challenging to handle. 
...
### Conclusion
Both MySQL and MongoDB are essential databases. Even after the emergence of non-relational data, relational databases are still important databases. It is unlikely that they will be extinct or even threatened by the emergence of non-relational databases.

It is the developer's job to decide, depending on the project scope, which database to use. The developer should choose carefully considering the requirements of the application and features of the database.

MySQL is flexible, has a higher performance, and is easier to manage data. However, suppose the developer comes across certain characteristics in a project with higher complexity of data, big sized data, growing data, and the need for object querying. In that case, it is advisable to change from MySQL to MongoDB.
