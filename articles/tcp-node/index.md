# Concepts of TCP, Explained (with Node)
...
Now that the server is done, we need a program to information. It needs to connect to the server.
...
The rest should be pretty easy. We'll ask for two numbers to add together, and send it to the server. Then, whenever we receive data back, we'll print the result and repeat the process.
...
There's already an article about [Socket.io](), so why do we need to know this?
