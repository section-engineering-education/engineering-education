...
Today websites are much like applications. You can send messages, transact, shop, and much more. In short, more interactivity is need on the user end, and hence server-side rendering can longer handle all this and once. It is taking a backseat and giving way to many advanced rendering technologies. There several major players here. This includes Client-Side Rendering (CSR) vs. Server-Side Rendering (SSR) vs. Static-Site Generation (Pre-rendering). Develops often find themselves confused about these terms. If you get yourself in this circle, this tutorial is for you. We'll discuss these technical terms, explore their pros and cons, and when to consider approach A or B.

### Client-Side Rendering (CSR)

Client-Side Rendering became popular after introducing the popular JavaScript framework, which started incorporating CSR in their style of development. They include [Angular](https://www.youtube.com/watch?v=xGpHfFf18Ns), [React](https://www.youtube.com/watch?v=w7ejDZ8SWv8), and [Vue.js](https://www.youtube.com/watch?v=qZXt1Aom3Cs).

In a client-Side Rendered website application, JavaScript controls what is displayed on the page. Typically, instead of loading all the web content using the HTML documents, a JavaScript file is included to handle the dynamic architecture of the website loading.
...
- The client browser will download the HTML content first and then JavaScript. These HTML files link the JavaScript. This loading happens as the sees the loading symbols as defined by the website developer, and the site is not visible to the user.
- After the browser downloads the JavaScript, content is dynamically generated on the client's browser.
- The website becomes visible as the client navigate and interact with the website.

This process means that the initial load time is prolonged. After initial load time, the web site's navigation will be super smooth and super fast, only having to make API calls to get the content dynamically.

The initial load is naturally slow as the browser is trying to download and load the initial run-time data for that website to the client browser.
...
The process involves fetching and processing data on the client-side (browser) and not the web server, hence the client-side rendering.

A great use case of Client-Side Rendered case is a single page application (SPA). In SPA, each page is rendered on the client browser. The server only serves one single HTML document. Once the HTML is loaded, the JavaScript frameworks such as [React](https://www.youtube.com/watch?v=w7ejDZ8SWv8) will control the website's DOM structure on the browser. In this case, each page will load from the data history as fetched by the framework's API. Once the initial load I over, accessing a different route or reloading the page will be superfast.

The JavaScript framework that supports client-side rendering development style includes [React](https://www.youtube.com/watch?v=w7ejDZ8SWv8) , [Angular](https://www.youtube.com/watch?v=xGpHfFf18Ns), and [Vue.js](https://www.youtube.com/watch?v=qZXt1Aom3Cs).

Let take a simple example of CSR using Vue.js.

There are two main files here, the HTML mark-ups and the JavaScript file. In most cases, content is wrapped inside containers `divs` with root `ids` to handle DOM to control states and data for the application.
...
In this approach, everything sent to the browser comes from a server. The server handles getting the web content and processing and displaying it on the browser. Every processing is done on the server every single time a request comes in, hence Server-Side Rendering.

Take a look at this example. Assume you have an imaginary server with an HTTP address `test.com`, and you are hosting the following `index.html` document as the main/ home page.
...
And that's not all. If you then click `Go to Home page`. The browser will hit the internet and make a request to the server. The server will render `index.html` afresh, regardless of this page was previously rendered. You get the point. Every page is independent and is rendered independently.
...
These generators prebuild HTML pages and make them available to the user ahead of time. This means that whenever a user requests a page, it loads with no delay. This happens so fast as the site is static and the generator renders the pages at build time.

A close relative of Static-Site Generators is content management systems (CMS) such as WordPress. A content management tool is used to generate and manage web content and web pages. They both use the concept of templates to avoid the writing, formatting of web content, and coding the webpages manually.
...
#### Client-Side Rendering
**Pros**
They are fast. Although the site's first load might be slow, once rendered, when a user requests a page, it's served instantaneously.

**Cons**
They have prolonged initial load time. This increases the likelihood of poor user experience, and user can leave you site when get frustrated having to wait for the CSR to fully render the website.
...
On SSR, the page reload, and visiting a new page has to hit a server request again. This comes with the burden of high memory usage and high processing power on the server. This consumes unnecessary internet bandwidth. This will obviously increase the hosting cost.
...
It has an overall slow rendering thus fits well when you have fewer users, complex UI, few pages, less dynamic data, and less interactivity.
...
They secure compared to dynamic websites. When you build a static site, you are giving the user accessing the website everything you have. They are no backend to hack or databases or text boxes to inject malicious code into.  A static website is plain and simple, just a collection of HTML files hosted on a server. Compared to other content management systems such as WordPress, content is hosted on databases on the backend, which can be hackable, and malicious activity can be injected into your website.

They are straightforward to host. They nothing to configure, no environments to set up. It is just as easy as uploading a couple of HTML files into a web server.

Hosting a static website is very cheap. They are no many resources to serve up a static website, hence low bandwidth and memory used, thus cutting the cost of hosting services. Hosting a web static can even be free. You can take all your static files and put them in free hosting services such as [Section](https://www.section.io/modules/nodejs-edge-hosting/) or the [GitHub pages](https://pages.github.com/).
...
I hope you find the blog helpful in understanding deciding which approach best fits your web content.