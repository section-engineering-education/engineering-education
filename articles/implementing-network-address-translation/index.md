...
Public addresses are routable addresses that are used on the internet,these addresses allow users to access resources on a computer network located anywhere in the world.Private addresses are not routable  and no traffic can be sent to them or by them over the internet.
...
3. Use the ```ip nat inside source list ACL number pool NAT POOL``` command to bind the ACL and the NAT Pool created.
...
### Configuring PAT with multiple public addresses
An organization was assigned two public addressees, `200.100.100.1` and `00.100.100.2`, and want to allow its internal hosts, in the private network `172.31.1.0  255.255.255.0` to reach the internet using PAT
