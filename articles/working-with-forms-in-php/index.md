...
- Understand PHP [POST and GET methods](https://www.w3schools.com/tags/ref_httpmethods.asp). Learn how the two methods work and how they differ from each other.
...
Collecting user's data in a web application using forms is a major task. In web development, there are several forms. Some of them include:
- Contact Forms
- Search Forms
- Login Forms
- Registration Forms
...
The GET method has a limitation of the amount of data sent from the form. The data being sent on the URL depends on the web server's operating system and the kind of browser. Most systems have a limit of 255 characters. The best example of using the GET method is with the search engine forms. The code below can be used to process an HTML form with a method set as `GET`.
...
First, create a database. Name it 'crud', in the database, create a table with three columns, name it 'user'. The columns are:
+ id
+ name
+ email
...
In the `update.php` script, a form with data matching the submitted is displayed for edits. Once the edit is complete, the updated data is submitted to the script for processing. In this case, we are using the same script to process update requests. The script below is used to update the data.
...
The button is a submit button for a form with a hidden field `id`. The data is sent to a form and processed by the PHP script below. If the `id` is not empty, then the record with the submitted `id` is deleted.
