...
Unlike in Q-learning where an agent has to learn the approximation of the value fuction and basing the policy on a direct estimate of the long term expected reward, Policy Gradient attempts to search the policy space directly in order to follow the gradient of the average reward. 
In addition, PG is able to express stochastic optimal policies and to be robust to small changes in the underlying function approximation.
