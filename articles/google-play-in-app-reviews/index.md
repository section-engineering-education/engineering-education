...
In this guide, we'll learn how to implement "rate this app" functionality within your android application context using Google Core in-app review API.
...
Reviews play a marketing role in your application. They are a crucial part of securing more downloads because they can tell a new user whether they want to download an application. There's a good chance a new user will check reviews and ratings to decide which application to try. The reviews give the experiences of previous users.

Reviews also form a line of communication between your users and the application, as they tell you what to improve in your application. Potentially, a user could catch a bug while interacting with your application. Feedbacks let you know that something might not be working well, giving you a nudge to fix a bug.
...
Another case would be that you have, say, an e-commerce application. In this case, it would be inappropriate to tell a user to review the app in the middle of making an order. You want the user to have enough information about your application services. In this case, you would ask a user to write a review when the order has been processed, to capture the right moment. You don’t want to show the review dialog when a user has not interacted with your application enough to give feedback.
...
2. This API also advises you only to show dialog once a user has enough experience with your application. It is not a good look to ask a user to write a review before the app delivers value to that user. Chances are, the user will get annoyed and skip the review; if they don't skip it, they will give negative reviews.
...
This API state's that if an error occurs during the review flow, you should never inform the user or change your normal application flow. The app will continue its usual user flow after onComplete Is called.
...
### Testing
To test our application, we will use internal app sharing. To confirm that this works, add a `Button` (a call to action) to trigger the review flow. Remember, we are using a call to action for testing. In production applications, calls to action such as `Button` should be avoided when using this API.
...
- Add `OnClickListener` call `startReviewFlow()`
...
I hope this guide is a helpful source for how to ask users for reviews as well as implement in-app reviews within your application context.