...
It is a class containing patterns used to check character combinations in strings.
It can be taken as an object in JavaScript.
...
`
We use the constructor function when we know the pattern keeps changing or like if we get it from a different source, such as the user input. For this reason, we are going to use the constructor function for we are getting the password from the user input.
...
We are going to discuss how to check the strength of  a password a user enters based on if the password:
...
A badge, basically a span, is used to show the password strength and color.
Green for strong, Blue for medium and Red for weak.
...
The complete code can be accessed in my [Github repository](https://github.com/Agusioma/Password-Strength-Checker/).
...
Hope you can manipulate the code more and give it more levels like Too Strong, Too Weak, etc.
You may even make it check the strength using percentages and animate the badge.
