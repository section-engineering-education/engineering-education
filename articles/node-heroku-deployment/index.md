...
You must have noticed that we had our `tokenSecret` as a variable in our files. This is not secure as it can easily be accessed and when in the wrong hands, it can be a great problem. In order to hide such values, we can store them as environment variables.
...
If you look at the url, there are two variables/values inside `<>` brackets. One is for the password of the user you created and the other one is for a database name. The database name can be anything you like. In my case I used `authentication`. And with that we have a remote database and a url to connect to it.Next, we modify the url in our `index.js` file. Change the url to the following.

```javascript
mongoose.connect(process.env.MONGODB_URL || dbURI , ...)
```

This tells the application to use `MONGODB_URL` and if not available, to use the `dbURI` value. To confirm the connection, you van handle the promise returned by the `mongoose.connect` method.
...
First, we do not want to push the node modules. We all know how large they are 😆. So go ahead and create a `.gitingore` file before you initialise git and paste the following code.
...
This even ignores our environment variables. Now run the following command to initialise and create an application in heroku.
...
But now when we run our application, we get an error. The reason is, we ignored our environment variables hence they were not pushed to heroku too. We have to move them to heroku too. Heroku also has it's way of saving environment variables. In heroku, they are known as `config vars`. And we can set them using the Heroku CLI. Let us upload our environment variables using the following commands
