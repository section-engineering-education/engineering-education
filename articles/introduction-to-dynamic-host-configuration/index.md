...
Router sends out a **Router Advertisement** (RA) message after receiving a router solicitation message. This message informs the IPv6 client of the availability of an IPv6 router or server to provide a prefix and prefix-length, and it is sent to the IPv6 all-nodes multicast address of `FF02::1`.
...
When a router receives this message, it replies with a router advertisement message to all nodes, with multicast address of `FF0::2`, providing a prefix length for `PC-A`.
