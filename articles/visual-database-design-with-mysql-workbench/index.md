...
#### Administration
Workbench enables the administration of MySQL Server instances. Some of the MySQL Workbench database administrative functions are:
- Backup
- Recovery
- Audit
...
**Step 4**
Select the objects you would like to be included in the EER diagram. These include tables, views, routines, users, and triggers. In this exercise, we only have tables. Make sure the option to import 4 table objects is selected, then click `NEXT`.
...
Reverse engineering enables us to have a better view of an existing database. A physical database is converted to an EER diagram. We will reverse engineer the database we created earlier. Reverse engineering can be done using MySQL Create Script or using a live database. In this tutorial, we will use the live database we created in the [forward engineering](#forward-engineering-with-mysql-workbench
) section. Follow the following steps:
