...
- Fits well when a business model has a rapid growth of data, and the database has no clear schema definitions, especially when you are dealing with unstructured information. Rapid data growth needs highly flexible data models to scale out the needs of that application.
...
- When dealing with millions and millions of rows and columns, NoSQL will be a significant consideration as it provides fast read and writes access to big data.
...
- Pre-defined data structure with multi-row transactions which have set schemas such as inventory system that have distinct entities. SQL will be the match as it provides perfect nominalizations and simple queries that comply with ACID
    ...
    - I (isolation) a transaction is securely and independently processed without interfering another transaction.
