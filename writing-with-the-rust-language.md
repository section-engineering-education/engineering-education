...
---
Rust is above all a systems programming language. What does this mean? For starters, this means that there is no runtime. In short, everything rust does uses zero-cost abstractions. You might be wondering how can rust implement abstract data types without adding runtime costs to the program.
<!--more-->

![](/assets/images/education/Rust_language_logo.png)

This article will talk about many of the interesting features that rust has to offer without really talking about how to write a program in rust. If that is what you are interested in, please refer
...
To note, there are other languages that are more comparable to Rust's features that I will not be talking about.
...
No, these changes must happen in the code because segfaults come from code, not the compiler. This also doesn't mean that the compiler can't help you write good code.
...
As a side note, take a look at how good rust's compiler errors are.
So why is this important? Well, if you were to try this in C you would get two references to the same point in memory.
This isn't necessarily a bad thing, but both of these references are mutable which can lead to race conditions. That is where the problem lies because C lets you easily have race conditions.
...
This might not seem super significant but what's happening is that rust is *forcing* us to write *good code*.
...
They do this by using their [enum](https://doc.rust-lang.org/book/ch06-00-enums.html) which is implemented using a C like union which can then be heavily optimized.
...
When it comes to using Options, you generally treat them the same as nullable values. The compiler will see this and optimize the `None` into null and the `Some(&T)` into a `*T` (this is only if we are using references to types e.x. `Option<&t>`).
...
## Can I just write Good C Code <!-- needs work -->
...
However, that doesn't mean it is good code. In the end, that's what rust helps with.
