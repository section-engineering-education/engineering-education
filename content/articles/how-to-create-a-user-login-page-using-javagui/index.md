...
- **Swing:** Swing is a set APIs containing GUI components used for creating Applications. Swing was build over AWT APIs. It provides a better and richer set of GUI components with a native or a cross-platform accessibility that makes the application look the same on different windows.
...
### Prerequisites
1. Basic Uunderstanding of Java and GUIs.
...
Once you have setup, your workspace should look like this:

![java gui](/engineering-education/how-to-create-a-user-login-page-using-java-gui/structure.png)

To famialirize yourself with Eclipse, it is recommended to read [this](http://people.cs.uchicago.edu/~kaharris/10200/tutorials/eclipse/Step_02.html) article.
...
In the example above, we declared a **button** as the variable name, **JButton** as the name of the class that holds the button component in AWT package, and **Login** as the text that appears on the button.

#### JPanel
JPanel class covers the entire space or a window where we attach the component to visualise it along with other panels.
...
> Note that, password textfields does not show the original characters. The english characters will be replaced by any other special character.

### Creating UI
Having understood a little about various Java GUI components. Let's start building the loging page.
...
Before we use JPanel, we have to create and instantiate object as shown:
...
From the snippet above, we instantiate a JPanel class using the variable name as `panel` . Then, we set the layout to be a `null` value, meaning: the layout should take the entire width and height of the screen.
...
Here, we check if the username is `section.io`, and password is `123`. To perform better validations, we must use Regular Expressions (REGEX).

`JOptionPane` is another components of Java GUI, it comes handy when displaying either a warning, success or error message on the screen, as a dialog box.
