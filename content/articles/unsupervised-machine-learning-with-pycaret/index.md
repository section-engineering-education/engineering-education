...
### Prerequisites
1. [Python](https://www.python.org/) installed in your machine.
2. Understanding of either [Jupyter Notebook](https://jupyter.org/) or [Google Colab](https://research.google.com/).
3. Understanding of  [Python](https://www.python.org/).
4. Latest version of [Pycaret](https://pycaret.org/) installed in your machine.
...
Use the following command to see our loaded data.
...
### Import Pycaret
We have to import Pycaret so that we can start using it to building our model.
...
### Initialize setup
We initialize our setup by using the `setup()` method. During initialization, ignore `country` and `name` columns, since we will not need them when building our clustering model.
...
- K-Means is a fast algorithm compared to the other algorithms.
...
After we have converted our third column from `str` (string) to an `int` (integer), we can start doing our scatter plot using the snippets below.
...
W- Use the `plot()` method to plot the world map, and it has a size of 20 on the x-axis and 10 on the y-axis.
