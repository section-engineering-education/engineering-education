...
In this tutorial, we will build a univariate time series model that predicts monthly milk production. We implement the model using a Recurrent Neural Network (RNN). More specifically, we will use Long Short-Term Memory(LSTM) which is a type of RNN best suited for time series modeling.
...
Recurrent neural networks use internal memory that helps them to store information retrieved from the previous data points and use this information to generate the next data points.

Recurrent neural networks implement a feedback loop within the layers with the hidden layer which distinguishes them from the traditional artificial neural networks.
...
Even though the recurrent neural network is a better improvement than the traditional artificial neural network, they suffer from a long-term dependency problem.  

A long-term dependency problem occurs when the sequential memory of the recurrent neural network and it not able to find the order of the data point. The sequential memory fails when feeding recurrent neural sequential data recorded over a long time such as a time series recorded for many years. 

The RNN will lose track of the order of the inputs and how the data points depend on each other. It can not process and analyze the dataset that has longer-term dependencies. To solve this problem, we introduce the Long Short-Term Memory(LSTM).

The image below shows the basic structure of an LSTM.
...
```
We use the `parse_dates` parameter to enable the Pandas library to recognize the DateTime in the `Date` column. It will not treat it like a normal string and we can therefore perform a time series analysis.
...
- `Residual`:  It shows the time series after the seasonality and trend components have been removed.
...
We have to convert the time series dataset into a scale of between 0 and 1. It will ensure that all the data points fit into the LSTM. We will use `MinMaxScaler` for scaling:
...
We have to take the scaled data points and break them into batches which we will feed the LSTM. The batches are a sequence of values that the LSTMS will use as inputs and then output the predictions after training.

We use the `TimeseriesGenerator` class to generate the dataset batches. We import this necessary library as follows:
...
n_features = 1
```
From the code above, each batch will have scaled data points for 12 months (n_input = 12). We then set the number of variables in the time series dataset using `n_features`. We set the value to one since we are dealing with a univariate time series.
...
We will build a sequential time series model using LSTM. A sequential model that will enable us to add multiple layers one after the other. 

We will import the LSTM from [Keras](https://keras.io/) and add it directly as the input layer of the sequential model. We will then add the output layer of the sequential model using the `Dense` layer.

Let's import the LSTM, Sequential model, and the Dense layer from Keras.
...
```
After this, let's add the `LSTM` as follows:

```python
lstm_model.add(LSTM(100, activation='relu', input_shape=(n_input, n_features)))
```
The LSTM layer will have 100 neurons. It applies `relu` as an activation function because the output of this layer will be between 0 and infinite time series values. It uses the generated dataset batches as inputs.

The model will then use the `Dense` layer to output the predicted time series values.
...
```
We compile the sequential time series model using the `compile` function. For compilation to work, we have passed the following parameters.
...
- loss - It is the parameter that will keep track of and store all the sequential time series model errors. We pass `mse` as the parameter value.

### Printing the summary of the compiled sequential time series model
To print the summary of the compiled sequential time series model, apply this code:
...
The summary shows the layers in the compiled sequential time series model and all its parameters.

Let's fit the sequential model to the generated batches.

### Fitting the sequential model to the generated batches
To fit the sequential model to the generated batches, apply this code:
...
We have trained the sequential model using the generated batches and the loss has reduced with time. Let's use the trained sequential model to make predictions.
...
```
The `rescale` method will rescale the testing time series values. We will the predicted output in the `prediction_result` variable.

We then use the following loop function to loop through the testing dataset. It will analyze the data points and make predictions.
...
If we observe the two lines, we can see both of them maintain the same pattern. Also, these lines are close two each other. It implies the sequential time series model was well trained and can make predictions that are close to the actual values.

### Conclusion
In this tutorial, we have built a univariate time series that predicts monthly milk production. We implemented the model using LSTM which is a type of RNN that perform time series modeling.

We explained the difference between RNN and LSTM and how the LSTM solves the long-term dependency problem. We imported LSTM from Keras which we used to build the sequential time series model. We finally used the line graph to evaluate the performance of the sequential time series model.
