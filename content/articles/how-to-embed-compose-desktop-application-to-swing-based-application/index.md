...
In this tutorial, we will introduce developing an app with Compose for Desktop and look at how it can interoperate with Swing. And integrate it with Swing components. 
...
- A quick recap of building Swing and Desktop Compose Applications
- Using ComposePanel to create UIs
- Composing a CFD using a Swing component

### Introduction
With Compose for Desktop, you can create UIs with Kotlin and handle Events in a responsive way.
...
Let's also have a look at how a Compose Desktop application is built like.
```kotlin
@Composable
@Preview
fun App() {
...
The swing will automatically view this component as one of its own.
...
- One of the benefits of 'SwingPanel' is that its position does not shift. When adjusting the position of the Swing component, decreases the possibility of problems.

### Using `ComposePanel` in a Swing application
Lets have an example of how our code should look like when using `ComposePanel`.
```kotlin
...
```
In the `main()` function, we created Swing components which are `JFrame` and `JButton` which we added in a `ComposePanel`. `JFrame` will render the window and `ComposePanel` will act as the container to hold all the components used. 
...
### Using `SwingPanel` for CFD Composition
You can also use `SwingPanel` to hold Swing components inside a Compose application. Without much a do, lets see how this happens.
```kotlin
...
### Conclusion
From the above tutorial, we can see that it is possible to use Swing components together with the Compose components. This makes it easier to migrate an existing Swing-based application to Compose application.
