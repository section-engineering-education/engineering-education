...
Use this [page](https://www.computernetworkingnotes.com/ccna-study-guide/download-packet-tracer-for-windows-and-linux.html) to download the Cisco packet tracer and install it on your PC.

### Terminologies
- *Classless routing* refers to a type of routing where the router is allowed to use the available default route to forward traffic if no other specific routes match, it includes subnet mask information in the routing updates.Supported by RIPv2, OSPF, and EIGRP protocols.
- *Administrative Distance(AD)* is a feature used by routers to select the best path possible, that is the routers determine the source of routes to take and use if it has two identical routes from different sources. Routers need to be able to determine which routes to trust if it's receiving the same information from two different sources. OSPF has a distance of 110, Routing Information Protocol (RIP) 120, and EIGRP	90.
- *Metric* refers to the ways used by each classless routing protocol to determine the best path to a network. OSPF uses the `Sum of Inverse of Bandwidth`, RIP uses `Hop Count` and EIGRP `Min. Bandwidth + Delay`.
- *Wildcard Mask* inverses of subnet mask configured on interfaces used by OSPF to specify the range of IP addresses to examine for a  match,usually 32 bit long.
...
- *OSPF neighbors* refer to the relationship that exists between two OSPF-enabled routers in the same OSPF area that are connected by a common network.
...
- Routing Protocol Messages - OSPF enabled routers to exchange messages to convey routing information using packets. i.e. Hello packets, link-state request packets, database description packet,link-state acknowledgment packet,link-state update packets used to discover OSPF neighbors to exchange routing information.
- Data Structures - the messages exchanged by the OSPF neighbors used to build data structures that contain a list of neighboring routers to exchange routing information with and are kept and maintained in RAM. They include `Adjacency database` - Creates the neighbor table `Link-state database (LSDB)` - Creates the topology table
...
As we have seen, we can use OSPF in classless routing in determining the fastest available routes for the packets from source to destination to help offer fast convergence and scales to much larger network implementations.
To summarize, we have:

- Learned what is  Classless Routing and protocols involved, OSPF protocol.
...
One can find more information about network management [here](https://study-ccna.com/ospf-configuration/).
