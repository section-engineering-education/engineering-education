 Imbalanced-learn is a Python package used to handle imbalanced datasets during machine learning. In an imbalanced dataset, the number of data samples is not equally distributed between the classes.

In an imbalanced dataset, the class labels are not equal. One class has a higher number of data samples. The other class label has a significantly lower amount of data samples.
...
When an imbalance dataset is used it will lead to model bias and inaccurate results. That's why we need to use various techniques and libraries to balance the dataset.

In this tutorial, we will use the Imbalanced-learn library to handle this problem. It will balance our sample dataset. We will then use the balanced dataset to build our model.
...
In any dataset, we usually have two variables, the `X` variable, and the `y` variable. The `X` variable represents all the columns that are used as input during model training.
...
From the output below we can see that we have an imbalanced dataset. The `active` class which is the `majority`, the class has more data samples as compared to the `inactive` class which is the `minority` class.
...
Lets import `train_test_split` package for dataset splitting.
...
```
The dataset will be split using a `test_size=0.2`. 80% of the split dataset will be the training set and 20% will be the testing set.

Let's have a look at the two splits.

```python
((462, 881), (462,), (116, 881), (116,))
```
Our training set is `(462, 881)`. It has `462` data samples and `881` columns. This is the set we will balance.
...
Imbalanced-learn has various techniques used to handle imbalanced datasets. In this tutorial, we will be focusing only on two techniques.  Random Undersampling Technique and Random Oversampling Technique.

#### Random Undersampling Technique

In the random undersampling technique the `majority` class which is the `active` class will be reduced.  This makes it have the same proportion as the `minority` class which is the `inactive` class.

The `majority` class has `330` data samples while the `minority` class has `132` data samples. In this technique, `330` data samples of the `active` class will be reduced to `132`. This will make the two classes to be balanced.

The random undersampling technique has functions and algorithms used to balance the dataset. Let's import one of the functions.
...
```
In the code above we specify the `sampling_strategy=1`. This ensures that the `majority` class and the `minority` class will have a 1:1 class distribution. We then add `X_train, y_train` which is the training dataset that we are balancing.
...
In the random oversampling technique the `minority` class is increased so that it's equal to the `majority` class. The `majority` class has `330` data samples while the `minority` class has `132` data samples. 
...
We will build our model using the dataset balanced by the techniques. We will then compare the accuracy score of the two models and see which technique is better.
...
```
We will use the `RandomForestClassifier` in building our model. We can now initialize our `RandomForestClassifier` algorithm before we start using it.
...
```
`matthews_corrcoef` will be used to automatically calculate the model's accuracy score.  It also helps in making predictions.

Let's apply the `model.predict` to make predictions .  `matthews_corrcoef` will output the accuracy score of the predictions. We are using the `X_test` and `y_test` in making the predictions.  These two variables contain the testing dataset.
...
df2
```
We have saved our model in the `mcc_test` variable. The code above will plot a diagram that will display the `Performance_metric_values`. Is shown in the image below.

![Accuracy score](/engineering-education/imbalanced-learn-python-package-for-machine-learning/accuracy-score.png)
The model accuracy score is `0.712435`, this is `71.2435%`. Let's use the next technique to build our model and see the accuracy score.
...
The model accuracy score is `0.744225`, this is `74.4225%`. This is higher compared to the accuracy score of the first approach. This shows the random oversampling technique is better than the random undersampling technique.

### Conclusion

In this tutorial, we have learned about the Imbalanced-learn. We went through the installation process of Imbalanced-learn. We also explored the various techniques used to handle imbalanced datasets.

We then implemented both the random oversampling technique and the random undersampling technique. We then build our model using the dataset balanced by the two techniques.

Finally, we concluded that random oversampling is the better technique. The random oversampling technique produces a model with a higher accuracy score.
