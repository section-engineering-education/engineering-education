...
In this tutorial, we will look at the various types of controllers. This includes the theory and the property of these controllers. The types of controllers that we cover are such as Proportional, Integral derivative controllers and combinational modes. Also, we look at how to design these controllers using Matlab.
...
- The performance of a system is usually analyzed based on a few parameters. These parameters are the time and the frequency domain. These parameters are analyzed in the image below:

![The parameters](/engineering-education/design-and-comparative-analysis-of-controllers-in-matlab/parameters.png)

- When we design a system, we get the transfer function. If, after testing, the value does not satisfy based on the rise time, overshoot, steady-state error, phase margin parameters, e.t.c, the controller is used to achieve the desired response.
- Controllers are system response modifiers.
...
1. Basic modes. These basic mode controller includes proportional(P), integral(I), and derivatives(D) controllers.
2. Combinational mode. These controllers are such as proportional+intergral(PI), proportional+deravatives(PD), and proportional+intergral+deravatives(PID) controllers.

Let us now look at these controllers.

### Basic modes
### 1. Proportional(P) controllers
A proportional controller is simply an amplifier. Its transfer function Gcs = kp. It does not add any new pole or zero to the system. The controller can be represented as shown below:

![Schematic represenation of P-controller](/engineering-education/design-and-comparative-analysis-of-controllers-in-matlab/P-controller.png)
...
With P-controller, the response of closed-loop systems becomes faster, but maximum overshoot also increases. The large value of `kp` can make the system unstable. Also, P-controllers cannot handle the steady-state error and provide offset error.
...
D-controllers give a fast action when the input changes rapidly. However, D-controllers are not used alone. They are not used alone because if the error $e_j(t)$ is constant, its output is zero, and the actuator output will also be zero. Also, if the error $e_i(t)$ changes suddenly, then the controller's output will be impulsive. This impulsive output can cause a sudden act of the actuator. This sudden act can cause a breakdown of the system components.

### Combinational modes
### 1. Proportional + Intergral(PI) controllers
...
In this case, $kp.e_j(t)$ is for the P-controller and $\frac{kp}{T_j}\int^t_0e_j(t)dt$ is for the I-controller. Here, kp is the proportional gain and $T_j$ is the intergral time. $\frac{1}{T_j}$ is the reset rate.
...
### Proportional + Deravative(PD) controller
...
### Proportional + Intergral + Derevative(PID) controllers
...
Finally, we make a closed-loop system with the controller. The variable `sys_cl_PI` is the system's transfer function with the controller.
To design other controllers, we perform similar operations as shown below:
"`Matlab
%---------------------------------------------------------------------------------------------
...
```
Now, we will compute the step response of the closed-loop response. This step response gives information such as rise time, settling time, maximum overshoot, e.t.c, in the workspace for the closed-loop system without controllers and for the closed-loop systems with PID-controller. Computing the step response is done by the code below:
"`Matlab
%---------------------------------------------------------------------------------------------
...
*Plot of the step response of different controllers*
...
*Plot of the response of different systems*
