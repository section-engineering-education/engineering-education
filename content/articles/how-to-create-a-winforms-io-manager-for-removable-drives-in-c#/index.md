...
In this tutorial, we will create a .NET desktop tool that shall enable us to enable or disable USB ports on our computers and also change the Read/Write permissions of the removable drives. We shall start by going through a detailed guide of creating the UI then we shall work on the code behind the above operations.
...
2. Basic knowledge in C# language and the .NET framework.
...
We shall start by opening Visual Studio and selecting `create new project` from the `Get started` options.
 On the next window(Create a new project) : 
...
![creating a new winforms project](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/create-a-new-project.jpg)
 ...
 we will select the `Windows Forms App` template then select `Next`:  
In the next window(Configure your new project) :
...
We shall rename our project to `IOManager`, leave the other fields with their default values and select `Next`.
In the `Additional information` window:

![Additional information](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/additional_info.jpg)

 we shall leave it to the default value and hit `Create`.
After loading the project, you should see something similar to this:

![initial window](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/initial_window.jpg)
...
![renaming Form1](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/renaming_Form1.jpg)

#### 2.2 - Changing the background color
Click the `I/O Manager` form and in the `Properties`  sidebar under `BackColor` change the background color. In this case, we shall change ours to `Teal`

![changing form1 background color](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/form1_bgcolor.jpg)
...
![form border style](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/form_border_style.jpg)
...
From the `Toolbox`, drag and drop a groupbox just below the instruction label and set its dimensions to `560, 150` under its size property. Change the `groupBox1` text to `Enable/Disable then change its background color to a more compatible color. We shall change ours to `ActiveCaption`.
...
![both radioButtons](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/both_radiobuttons.jpg)
...
![button properties](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/btn_properties.jpg)
...
![with all buttons](/engineering-education/how-to-create-a-winforms-io-manager-for-removable-drives-in-c#/with_all_buttons.jpg)

### Step 3 - Adding Code to the Form
#### 3.1 Adding required namespaces
To edit the code, right-click on the form(the teal background) then select `view code` and Visual Studio will load `Form1.cs`. Now we will add the namespaces commented `new` just below the default ones :
```C#
using System;
...
`Microsoft.Win32` - provides us with a class that handles events that manipulate the system registry.
`System.Runtime.InteropServices` - supports COM interop and platform invoke services.
...
```
The strings contain paths to devices configurations ie: windows registry.
...
```
The `Cancel` button(button1) is used to uncheck the checked radiobuttons . Here is its code:
```C#
