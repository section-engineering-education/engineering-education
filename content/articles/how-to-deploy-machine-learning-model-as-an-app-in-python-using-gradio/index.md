...
- Know [Python programming](https://www.section.io/engineering-education/python-projects-for-beginners/).
- Know how to work with [machine learning models.](https://www.section.io/engineering-education/model-interpretation-using-lime/engineering-education/house-price-prediction/)
- Know how to use [Google Colab notebooks.](https://colab.research.google.com/)

### Introduction

Gradio integrates with most Python libraries. These Python libraries include: 
- Hugging face: 
It's an open-source library used to build natural language processing applications.
- Matplotlib: 
It's a Python library that is used for plotting diagrams during machine learning. Used for visualization.
- Numpy: 
It's used for data analysis and manipulation.
- PyTorch: 
It's used for computer vision and natural language processing.
- Sckit-learn: 
It provides various classification, regression, and clustering algorithms for machine learning.
- Tensorflow:
 It's used to perform tasks for machine learning and artificial intelligence.
...
When you launch a Gradio interface, it automatically generates a public link. This link is sharable. It allows other people to interact with your application remotely. The link expires after 72hours. 
...
During use, you can easily identify bugs and errors in the model. This enables you to remove the bugs before production.
...
```
We import Gradio using the following command.

```python
import gradio as gr
```
Let's launch a Gradio using a simple function. In the code below, we have created a function `greet` that will return `Hello` and the `name` the user inputs. 

```python
def greet(name):
 return "Hello " + name + "!"
```
Let's create a Gradio user interface to display the results. We create the user interface using the following code.
...
- `inputs`
These are input component types that they enter in the Gradio interface. In our case, it's a `text` type.

- `outputs`
The ouput after processing.
