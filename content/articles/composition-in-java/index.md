...
As you can see from the above class Vehicle, we have treated the child class Engine as an attribute though not. So we have initialized it, created a constructor for it, and created a getter and setter for it, just like any other attribute. So let us now create the Main class where we will run and implement the code. 
...
3. With composition, we can re-use code even with the final class, but it cannot be achieved with inheritance. 
