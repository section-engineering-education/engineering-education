...
### Introduction
In designing road layout, we use the `drivingscenario` function. `scenario` function allows for simulation of an activity or a structure that can be found in the real world.

`Drivingscenario` function can be used to create different designs of road layout by plotting the road layout in 2D or 3d graphs. This is done by specifying the coordinates of the road center using function `roadCentre=[];` and the road width then plotting these points on the graph.
