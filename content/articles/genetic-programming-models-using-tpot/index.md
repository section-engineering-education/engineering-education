...
In this tutorial, we will use two approaches in building the machine learning model. First, we build a model without applying genetic programming. We will follow all the traditional stages from dataset pre-processing to model building.
...
There are three concepts used by TPOT during the genetic programming process.
...
In the next section, we will use TPOT in building a machine learning model. TPOT combines all the available classification algorithms and finds the optimal one. The one with the highest accuracy score is chosen.
...
Import a cross-validation score. Is used to test the effectiveness of our model and its ability to make new predictions.
...
The model will iterate 10 times and find the average accuracy after the 10 iterations. The model learns from the dataset and improves over time.
...
We will do the same 10 folds in building our model.
...
TPOT combines all the available algorithms for classification and finds the best one. It automates the process of the model building using genetic programming. This saves a lot of time manually comparing all the available algorithms.

In the next section, we will see how to build a machine learning model using TPOT.

### Model building using TPOT

To use TPOT we install it into our machine. We install TPOT using the following command.
...
We initialize our TPOT application using the `TPOTClassifier()` method. We pass the following parameters.
...
> By the end of the 5 iterations, we will get an output with the best algorithm to use for model building.
> TPOT will also give us the highest accuracy scored by our model.

Useful since it saves the users time by automating the whole process. During this optimization process, TPOT uses the concept of genetic programming. It eventually finds the best algorithm.

Only the best algorithm will be chosen, the poor algorithms will be dropped. After the optimization process, the output is as shown.
...
TPOT also helps us know the exact parameters used to achieve such optimization. The parameter used is such as `n_neighbors=20`.
...
TPOT takes the highest score as the model accuracy. The final model accuracy will be `0.9714285714285715`.
...
In this tutorial, we have learned about genetic programming. Genetic programming allows the model to evolve over time. This enables us to get the best algorithm for the model. We have also learned about TPOT. It's is an important library used for automation during genetic programming.
