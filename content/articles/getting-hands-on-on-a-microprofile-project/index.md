...
- A modern IDE that supports Java frameworks installed on your machine. This may either be Visual Studio Code or IntelliJ (Ultimate edition) or any other.
  We shall focus on the two in this article.
- A stable internet connection for dependency downloads.
- JDK 11+ of any distribution installed. This is because MicroProfile is not supported with lower versions than 8.
...
It defines how microservices are built, work, communicate with each other and how they are made to be independent but loosely coupled to promote application portability.
...
| [Quarkus](https://quarkus.io/)  | Is a Kubernetes Native Java stack built for GraalVM and OpenJDK HotSpot, crafted from the best of breed Java libraries and standards.  |
...
| [Helidon](https://helidon.io/#/)  | This is a collection of Java libraries used for writing Java microservices which run on a fast Helidon Reactive WebServer, a web core powered by Netty, while supporting MicroProfile and its common specifications  |
...
In it, you will see some options such as the _groupId_, _artifactId_,_MicroProfile Version_, _Java SE Version_, _Build Tool_ (i.e. Maven and Gradle), _MicroProfile Runtime_, and _Examples for Specifications_.
Once these are set, you can be able to download the zipped project. Just unzip it to get the project as you would expect.
...
As of the 4.1 version, the notable Microprofile API's specifications, which are 18 in number are divided into two main categories; the ones for MicroProfile applications (13), and stand-alone applications (5).
...
| Open Tracing | It defines an API and associated behaviors that allow services to easily participate in a distributed tracing environment |
...
Head over to the zipped file and extract it. Open it up with your IDE.
In case you are using Visual Studio Code, head over to the section below to see some extensions you will require installing to support your project.
...
Once done, head over to the Command Palette by using `Ctrl + Shift + P` or find it in the Menu bar under _View_.
...
Wait patiently as it downloads all needed dependencies and also builds the files.
...
This shall run the service in the dev mode which supports features such as 'hot reload'.

Accessing the application via `http://localhost:8080` in the browser.
It shall look as shown below:
...
Do this by opening the document using a text editor then copy-paste the text into the site and access the endpoints using the Graphical User Interface (GUI) found on the side of the page.
...
As for Open Tracing capability, done using [Jaeger](https://www.jaegertracing.io/download/), just head over to the links provided in the page or 'readme.md' file to configure it and run it.
...
Some servers used include:
Quarkus, Payara Micro, WildFly, Apache TomEE, Hammock, Open Liberty, Helidon, KumuluzEE, and ThornTail.
...
- How to view the OpenAPI documentation via an online Swagger OpenApi editor and also locally by adding it as a dependency.
