...
An important point to note when working with geodesic or when calculating any distance between any two points on the sphere is that it does not ignore any curved part. Geodesic distance measures distance in such a way that it includes even the curved part. In contrast to this, a distance calculation known as Euclidean distance completely ignores the shape of the path that is involved such that it assumes a straight path from the start of a point to its end.

We'll illustrate how to compute or how to calculate the Geodesic Distance using latitude and longitude data in the next example.
