...
In this guide, we will look at some of the strategies Spring Boot provides us to deal with this challenge. We will look at how to change environments, extract environment variables, and more. By the end of this article, you will be more equipped to deal with configuration challenges.
...
When talking about environment configuration, it is essential to understand what profiles are. A profile in Spring is the name of the environment you want to run your application in. The names for your profiles can be whatever you want and can represent any environment you want. Two prevalent ones would be a development and production environment.
...
Spring Boot provides a simple solution to this problem using externalized configuration. All you need to do is create a new config file in the same directory as your jar file. You can use this new config file to override any configuration inside the jar. Note that your command line needs to be pointing to the folder your jar is in when running the jar in this case. I believe this is because it looks at the current directory you are at for the externalized configuration.
...
So far, we have looked at how to set config variables based on your profile and extract them in your code. Now we’ll take a look at how you can change behaviors within your code based on the profile you are in. I’ve rarely found good use cases for this, but it can be helpful when the time comes.
