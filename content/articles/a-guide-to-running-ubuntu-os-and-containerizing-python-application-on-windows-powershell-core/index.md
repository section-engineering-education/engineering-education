...
status: publish
published: true
url: /a-guide-to-running-ubuntu-os-and-containerizing-python-application-on-windows-powershell-core/
title: Running Ubuntu Machine and Containerizing Python Application on Windows Powershell Core
description: This article will teach you how to run Ubuntu Docker Machine and Containerize Python Application on Windows Powershell Core
author: arafat-olayiwola
date: 2022-04-30T00:00:00-05:20
topics: [Languages]
excerpt_separator: <!--more-->
images:
  - url: /engineering-education/a-guide-to-running-ubuntu-os-and-containerizing-python-application-on-windows-powershell-core/hero.png
...
2. Guide to containerizing Python Flask application


### Setting up the Ubuntu OS on Windows Powershell Core through Docker containers
Historically, different operating system configurations play a major role in the application testing environment; due to dependencies required by applications, a change in environment when running affects its behaviour and performance. Setting up the Linux environment on Windows can be done in multiple ways. Here we will discuss the major way to use the ubuntu distro container.
...
In addition, the Ubuntu virtual machine will start globally inside the directory `/bin/bash`. Therefore, before you begin executing various Linux commands, ensure that you update the packages by pasting the below snippet.
...
Application containerization is widely adopted because it helps in packaging dependencies as required and runs without any extra configuration in any environment with Docker installed. In the past, developers worried about the operating system differences when starting an application developed on a different platform.

The most widely used containerization platform is the `Docker`. It solves the problem of operating system differences due to the various dependencies needed by the newly made application. To containerize an application, we need to be familiar with the following terms;

1. ##### Dockerfile
    It is the file required to build the application image on top of the pre-built image of the language used. The file highlights step by step instructions needed.
...
Having introduced the above terms, we can begin with the steps required to containerize a Python application. In this project, you will be making Flask `drinks` API endpoints that return all mock data.
...
- Note: The `code .` opens the current directory in visual studio code editor.
...
Now you need to build the application `image` from the Dockerfile. This is achieved through the `docker build` command with the' latest-image' tag. 
...
```
Every request to `localhost:8000` is forwarded to the container port, and the application displays the index page. The container detects the type of image for a specific request using both the port and the tag.
