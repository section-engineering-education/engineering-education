...
In networking, the terms "Switches", "Hubs", and "Routers" are sometimes used interchangeably. This is wrong. Despite them being similar, there are differences in how they handle data. These three components may be integrated into a single device making it hard for a student to distinguish between them.
...
- [Layer of each component in the OSI model](#layer-of-each-component-in-the-osi-model)
...
All information sent to a hub is automatically sent to all ports of the devices connected to it. Thus, it does lots of wastage of bandwidth. 
 
### Advantages of using hubs

 - It has the ability to connect to the network using different physical media.
...
- Hubs share data to all the devices in a network thus making the network not secure.
...
- The have low bandwidth because of its dynamic router communication.
...
- When a hub receives a data packet (an Ethernet frame) from a network device at one of its ports, it broadcasts (repeats) the packet to all of its ports to all other network devices. A collision occurs when two network devices on the same network tries to send packets at the same time.
...
- **It manage traffic** - By forwarding data packets to their proper IP addresses, traffic between these networks may be managed.
...
It supports half-duplex transmission. Thus, only one node can send data at a time.

### Addresses used in each device
#### Switch

A switch stores and uses the MAC address of a device to transfer data.

#### Router

A router uses the IP address of the device to transfer data between networks.

#### Hub

A hub does not store any MAC/IP address to transfer data.

### Transmission of data
#### Switch
A switch transmit data from one device to another in form of [frames](https://en.wikipedia.org/wiki/Frame_(networking)).
...
In this article, we have looked at what is a hub, switch, and router. We have also looked at the functionalities and applications of each device as used in networking. Hope the article has been of help to you both personally and academically.
