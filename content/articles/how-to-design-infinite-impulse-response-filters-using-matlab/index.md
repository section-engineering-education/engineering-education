...
The other name for the Butterworth filter is a maximally flat filter. It is because it is designed in such a way that the frequency response is as flat as possible in the passband. Passband is defined as the range of frequencies that are passed through a filter. The frequency response is the transfer function of a given filter. Butterworth filter achieves its maximum flatness as it changes from passband to stopband at the expense of a wide transition band. It is a disadvantage since it results in poor characteristics. A transition band is the range of frequencies that allows transition between the passband and the stopband. The two types of Butterworth are;
...
Then,  passband ripple, which is the range of the amplitude in the filter's passband.
- passband frequency - as defined earlier, it is the range of frequency that passes through a filter. It means that it only allows signals of a given frequency to pass.
...
After defining our filter, we need to normalize the filter. Normalization is changing the component values to a suitable frequency. To remove undesirable characteristics, we normalize the filter. To carry out normalization of we filter, we use the code below;
```Matlab
wp = fp/(fs/2);
ws = Fs/(fs/2);
```
The next activity is the determination of filter order and the filter cut-off frequency. The transfer function defines filter order. The cut-off frequency is the frequency at which the response becomes too weak. To determine this, use the code below; 
```Matlab
