...
They allow developers  process a collection of data in a declarative manner. This makes code clean and concise.
...
We have created a stream using the `Stream.of()` method in the above example. We have passed two list-objects. That is, List of `fruits` and `drinks.`
...
### Wrapping up
In this tutorial, we have looked at the basic concepts of streams. For instance, we have learnt how to create a stream from a collection. Java streams represent a data pipeline and the functions used to manipulate the data.
