...
In our code below the developper class  define a constructor function .The developer class expects the name argument and it uses `this` keyword to storethat value on the instance. A new object is generated by the `new` keyword.dev1 is returned as the new object hence it outputs `Tom` as our new developer.
```js
   class Developer {
  constructor() {
    this.name = 'Tom';
