...
In our code below the developer class  define a constructor function .The developer class expects the name argument and it uses `this` keyword to store that value on the instance. A new object is generated by the `new` keyword.dev1 is returned as the new object hence it outputs `Tom` as our new developer.
