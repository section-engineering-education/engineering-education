...
Furthermore, since your server's email does not originate from a well-known email service provider, it is less likely to be rejected or branded as spam when sent to many email accounts.
...
For GitLab.com, enter the user `Password`. When you're done, click on Mirror repository to complete the setup.
