...
When it comes to perfomance, YOLO outperforms other object detectors by far. In real-time, it is able to process images at a rate of around 155 frames per second, achieving double the mAP(Mean Average Precision - *a popular evaluation metric for object detectors*) of other object detectors like R-CNN.

YOLOv5, the latest release of the YOLO family is a group of compound-scaled object detection models trained on the COCO dataset used for model ensembling(*combining multiple models in the prediction process*), Test Time Augmentation (*performing random modifications to the test images e.g flipping, rotating etc.*) and hyperparameter evolution(*optimizing hyperparameters using a Genetic Algorithm for optimization*).
...
To test its peformance on other sources like an image file, use the following command:
...
```
Here, we are using yolov5 pretrained weights to train images at a native resolution of `--img 640` (size 640 pixels) from source `data/images`.
...
Here, we finetuned COCO128 for 3 epochs using pretrained YOLOv5s.
