...
Next, we add an `on click()` method to the form so that when we hover over the form, we can click any place to call a function to select the file we upload to a buffer. 
