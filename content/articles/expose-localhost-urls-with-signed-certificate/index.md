...
description: This article aims at building a custom Django application that exposes endpoints and configures a self-signed SSL certificate to ensures that the endpoints are secured. Finally, it will direct the reader on how to expose the secured endpoint using Ngrok.
author: phina-kersly
...
![Ngrok Generated URL](ngrok-generated-url.png)
...
![Secure endpoint images](secure-endpoint-images.png)

### Conclsuion
We wrote a Django REST API using the Django REST framework in this article. First, we tested our API locally and later exposed it using Ngrok. Next, we installed a custom Certificate Authority in our local machines and then generated a self-signed certificate to secure our localhost endpoints. Lastly, we tested the secured endpoints to see if they were secured.  
