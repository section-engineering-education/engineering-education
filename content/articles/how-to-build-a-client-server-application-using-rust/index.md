...
The reader will be able to understand the concept of client-server communication. The reader will then learn how to achieve this using Rust.

### Set up Rust
Rust is a very low-level/systems-level programming language like C++ or C. it focuses on safety speed and concurrency, ensuring you have a design that lets you create programs that are high performant. To start using Rust, you need to get a Rust C, which is the Rust compiler.
...
>Alternately, you can install Visual Studio 2019, Visual Studio 2017, Visual Studio 2015, or Visual Studio 2013, and during install select the "C++ tools": `https://visualstudio.microsoft.com/downloads/`.
...
This will install along with the Rust cargo. Cargo Rust package manager allows you to have access to remote Rust packages. Cargo is closely related to NPM for Node.js, composer for PHP, or PyPI for Python.
...
- To check if Rust's Rust C is the compiler is installed rn;
...
We need a couple of Rust modules to set up this server and client. This includes;
...
Add the above modules to your `main.rs` file, as shown below.
...
Before setting up a server, the server needs to handle the client's messages. The server has first to access these messages then decide what operation to perform.
...
fn handle_sender(mut stream: TcpStream) -> io::Result<()>{
    ...
    let mut buf = [0;512];
    for _ in 0..1000{
        // let the reciever get a message from a sender
...
The serve will constantly loop around the connected clients. This can create permanence overhead. Thus we need to set a `time::Duration` that allows the server to `sleep` for a moment when looping through the sent messages.
...
fn main() -> io::Result<()>{
    ...
    for stream in receiver_listener.incoming() {
        ...
        // let the reciever connect with the sender
        let handle = thread::spawn(move || {
            //reciever failed to read from the stream
...
Once the connection is made, we want to basically set what should happen when our server receives a message. We are going to collect all the incoming messages to our stream and bind them to a server socket address. We then put these messages into a heap in the order they are sent. The server will then read and write each message from the set heap stack.
...
fn main() -> io::Result<( )>{
    ...
    for _ in 0..1000 {
        ...
        // Write the message so that the reciever can access it 
        stream.write(input.as_bytes()).expect("failed to write");
        // Add buffering so that the reciever can read messages from the stream
...
First, we will create a mutable client with `TcpStream` and then connect it to a localhost IP with a port number. Here we can have different clients connecting to the same server. Thus we need to create a mutable `TcpStream` to allow different localhost IP address to connect to the server.

Once the client connects to the server, it needs permission that allows the sender to enter a mutable message input. The server is now supposed to print this message. These messages are read string, which is then converted to a `utf8` buffer.

Now just need to add buffering so that the server can read messages from the client stream.
...
Ensure you server is still up and running in a different terminal. Head over to the client terminal, start typing some messages and click enter to send.
