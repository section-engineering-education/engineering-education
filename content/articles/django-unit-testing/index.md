Testing is an important step in the software development process. Many software developers ignore this step and test their code manually. Manual testing of code becomes tedious as the application size grows. Unit tests ensure that every component you add to your application works as expected without breaking the functionality of other components.
...
```
The above code snippet contains a Student model that will be created in the database as a student table. The model has two methods:-
1. `get_absolute_url(self):` returns the URL to a specific student detail page.
2. `__str__(self):` returns a string with students' first name and last name.
...
```
The above code snippets convert the `Student` model to a JSON and vice versa. JSON data is easy to transmit over HTTP, that's the reason why the data is converted to JSON.
...
2. In the `templates` directory created above, create another directory within it named `testing`, this will hold template files for the `testing` application.
