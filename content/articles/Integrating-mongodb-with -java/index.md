...
Collections are how MongoDB organizes documents. In relational databases, collections it makes use of tables.
[Differences between collections and tables.](https://docs.mongodb.com/manual/core/databases-and-collections/)


#### No schema definition is required in MongoDB, but a clear schema is required in RDBMS.
MongoDB allows unstructured data to be stored that is building your application without first defining the schema. Before adding data to a relational database, you must first define your schema and this affects cchanging the schema structure of a relational database as it'll be expensive and time-consuming. A schema is the structure of the database that is described in a formal language supported by the database management system. Since MongoDB does not require a predefined schema, it is much easier to update as data and requirements are changing.
...
![Available Databases](/engineering-education/integrating-mongodb-with-java/available-databases.png).

#### Connectivity
In this tutorial, you'll create a database called SchoolManagementSystems and populate it with a collection called Students. The schoolManagementSystem in simpler terms is the database name while the students will be the table name. But since in MongoDb there is no use of tables youll use collections and it'll be named students. 
...
![Created Collections](/engineering-education/integrating-mongodb-with-java/created-collections.png)
...
![Inserted Data](/engineering-education/integrating-mongodb-with-java/insert-data.png)
...
![Updated Data](/engineering-education/integrating-mongodb-with-java/update-data.png)

### Delete data in the collection.
To delete any unwanted data, the delete function is used as shown below.
```java
//it deletes the selected data inside the collection
...
![Deleted Data](/engineering-education/integrating-mongodb-with-java/delete-data.png)

### Delete the collection
 To delete a collection that has been created the code below is used.
 ``` java
 //it deletes the whole collection
  collection.drop();
 ```
...
Java and MongoDB are two of the most popular software development technologies, so it's no surprise that they complement each other well. Whether you're developing on a local or on-premises MongoDB installation or going cloud-first with MongoDB Atlas, the MongoDB Java Driver and surrounding ecosystem make integrating MongoDB into your development process a breeze. In this tutorial, we have looked at setting up a MongoDB server. We have also looked at how to establish a connection between MongoDB and Java and how to operate with different CRUD operations. I hope you found this article helpful.!
 [This Github URL will be of help](https://github.com/dboissier/mongo4idea)
