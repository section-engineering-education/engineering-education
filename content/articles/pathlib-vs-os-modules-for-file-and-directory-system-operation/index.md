...
In this tutorial, we will run through different viable contexts to discover an agreeable decision to utilize Python's Pathlib or OS packages for file system operations.

In this tutorial, we will learn about various file system operations - not limited to writing and reading a file, describing a document information, renaming files, creating directories and so on.
...
Querying the current working directory is a basic task when you are dealing with a path.

With these libraries, you can fetch relative path and absolute path.
...
> Note that, your output will look different (contains forward slash `/`), if you are not using Windows OS.
...
Helps navigate easily navigate to the home directory from your current directory.
...
> Note: The `os.mknod()` function is only available on a Unix type operating system which illustrates that this function is not available on a Windows operating system.
...
It returns `True` because the directory exists. It would return `False`, if the directory does not exist.
...
##### Utilizing an alternative way of renaming a file or directory using pathlib
```python
from pathlib import Path
...
OS and Pathlib modules provide a method or function that conditionally deletes a directory.
...
> Note: There are alternative ways we can use to delete a directory that contains files or a file, possibly you can use the `os.remove()` function provided by os module or `Path.unlink()` method provided by pathlib to remove a file one after the other.
...
The output covered that the file has been deleted.

>Note: You can as well attempt to iterate through the current directory at your end since you have been shown how to do such.
...
Any type of operation related to working with file information can also be called Metadata, in this section, we will be using the time and datetime modules to convert/interpret a given file's information.

> Note: Both OS and Pathlib modules provide function/method for performing this type of operation. OS and Pathlib modules perform exactly the same operation and both return a `os.stat_result` object.
...
In the example above, we simply queried the corresponding attributes to get their distinguished state representing the file metadata.

The `st_size` contains the file size in bytes, the `st_atime` contains the date of the most recent access to the given file in seconds, the `st_mtime` attribute contains the file last modified date in seconds, the `st_ctime` attribute respond differently on Unix and Windows operating system, for windows it contains the creation date, as for Unix it contains the changed date, either you can use `st_birthtime` for querying creation time on Unix operating system.
