...
Dropout regularization is a technique that randomly drops a  number of neurons in a neural network during model training. This means the contribution of the dropped neurons is temporally removed and they do not have an impact on the model's performance.

The image below shows how dropout regularization works.

![Droput regularization](/engineering-education/dropout-regularization-to-handle-overfitting-in-deep-learning-models/spark-streaming.png)

*[Image source: Medium](https://miro.medium.com/max/648/0*EY8R7nS10y5kQzOx)*

In the image above, the neural network on the left shows an original neural network where all neurons are activated and working. On the right, the red neurons have been removed from the neural network. Therefore, red neurons will not be considered during model training.
...
- The neurons can't rely on one input because it might be dropped out at random.
...
This ensures only important information is stored by the neurons. This enables the neural network to gain useful knowledge which it uses to make predictions.
...
The dataset used is the [Sonar dataset](https://datahub.io/machine-learning/sonar). The dataset is used for the classification of sonar signals using a neural network. The model will be used to predict if sonar signals bounced off a metal surface or a rock surface. To download the CSV file, click [here](https://datahub.io/machine-learning/sonar#resource-sonar_zip)

To load the dataset that you have downloaded, we use Pandas and Numpy. These are useful Python packages used for dataset manipulation and analysis.
...
From the image above, the dataset has a total of 61 columns labeled from 0. The first 60 columns are the input columns for the model, the last column(60th) is the output column. The last column is either labeled `R` or `M`, this represents rock and metal respectively.
...
y = df[60]
```
From the code above, the X variable is the first 60 columns and the y variable is the last column(60th). The last column is either labeled `R` or `M`, but we need to convert it to a number format. It should be either `0` or `1`, `1` will represent `R` while `0` will represent `M`.
...
![Converted columns](/engineering-education/dropout-regularization-to-handle-overfitting-in-deep-learning-models/dataset-structure.png)
...
```
From the code above, we have imported `train_test_split`, this is the Python package used for dataset splitting. We split our dataset using a `test_size=0.25`. This ensures that `75%` of the dataset will be used for testing and `25%` will be used for testing.
...
```
We will use both `TensorFlow` and `Keras` in building our neural network. `TensorFlow` will run our backend when training the neural network. `Keras` will create the input, hidden and output layers of our neural network.

In TensorFlow, we have two types of models, a sequential model, and a functional model. In sequential models, layers are built on top of each other one after the other as shown in the image below.
...
Functional models are more flexible since they do not attach layers in sequential order. We will build a sequential model.
...
- 1st layer
The first layer is the input layer for our neural network. It has `60` neurons. We also add `activation=relu`. We use `relu` because our output lies between 0 to infinity.

For more information about `relu` activation function, click [here](/engineering-education/activation-functions/)

- 2nd layer
This is the first hidden layer, it has `30` neurons. We also add `activation=relu`.

- 3rd layer
This is the second hidden layer, it has `15` neurons. It also uses `relu` as the activation function.

- 4th layer
The 4th layer is the output layer. It has only `1` neuron which is used to produce the prediction results of the model. The layer uses `sigmoid` as the activation function. `sigmoid` activation function is used when the output values that between `0` and `1`. 
...
From the image above, the training score after `100` iterations is `1.00`, this represents `100%`. To know if our model is overfitted, let's use the testing dataset to calculate the accuracy score.
...
- 1st dropout layer
This layer is added after the input layer. We set the number of neurons to be randomly dropped to `0.5`. Therefore, half of the neurons will be randomly dropped from the input layer.
...
From the image above the training score, after `100` iterations are `0.9167`, this represents `91.67%`. To know if this process has handled overfitting, let's use the testing dataset to calculate the accuracy score.
