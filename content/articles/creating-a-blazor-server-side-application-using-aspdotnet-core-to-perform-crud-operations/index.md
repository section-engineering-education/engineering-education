...
![N ame of app](/engineering-education/creating-a-blazor-server-side-application-using-aspdotnet-core-to-perform-crud-operations/nameofapp.png)
...
Now run run app to see how it looks like in a web browser. It should look like this.
...
You will implement the method above in the `StudentService` file. You will create a private property of field so as to utilise the list of students by creating a private list of students and return the list as shown below in the code snippet below.
...
In order to use the method above, you need to inject the service in the `Student.razor` file using the `inject` key word by calling the `StudentsService` so that you will be able to use it anywhere in the code.
...
The `@page "/students"` is a url that matches to this component to be accessed by the razor page.
...
Also in in the `startup.cs` file, you will add a singletone of `StudentService` such that when you ask for `IStudentsService` it gives you the StudentService.i.e;
...
Since you dont have a list of students, you will have to create a students model, and give it the following two properties.
...
Now you will need to utilise the Id above and fetch the student data so that you can be able to edit the details. To do that, you will go to the student service interface and create a method to fetch the student details.
...
Now you have populated the name and Id of the student list, you now need to create an update button so that you can update a student and route back to the students list and be able to see the change.
...
Now you will create an an anchor tag in the `student.raezor component` file to link the pages.
...
In the submit form, you will check if the student is empty then you will call the student service to add the student, otherwise you wil just update the employee.
...
Now, in the studdent service interface, you will create a method to delete a student and implement the method in the student service file by fetching the student data and removing it from the list.
