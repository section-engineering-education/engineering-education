...
title: Side-Effects and Effects Handling in Jetpack Compose
description: This tutorial provides a guide on how to use the Jetpack Compose EffectsHandler APIs to handle side-effects and effects in Compose.
author: samantha-namenya
date: 2021-12-08T00:00:00-14:14
topics: [Languages]
...
---
A side effect is everything that escapes the scope of a function. For compose specifically, this refers to everything inside a composable function that has nothing to do with compose.
<!--more-->
To handle such effects, we use an `Effect Handler` which is a safe environment to handle side-effects that may occur.

### Prerequisites
To follow along with this tutorial, the reader should:
- Have an understanding of the `Kotlin` programming language.
- Have a basic understanding and experience with `Jetpack Compose`.
- Have a basic understanding of the [composable lifecycle](https://developer.android.com/jetpack/compose/lifecycle).
...
### Introduction
There should be no negative consequences to utilizing composables. Effects should be invoked from a controlled environment that knows the lifecycle of the composable when they are needed to modify the app`s state.

The Effect APIs are used when you need to modify the state of the composable so that side effects are executed predictably. Let's get started and see how this happens.

### Side-Effects and how they occur
As we have seen, a side-effect is anything that exists outside the scope of a composable function. Side effects can cause adverse effects to an app. This is because they are capable to modify the state of the application beyond the scope of the composable. This implies that every time a composable function is invoked, it may respond differently.

Composable functions run several times or even skip execution. This means that they cannot be completely depended on. Having to rely on their global state can result in unexpected behavior.

Compose has several strategies for dealing with side effects. These techniques assist in regulating the lifespan of side effects and removing them after they are no longer needed.

There are several ways we can perform side-effects in Compose.

### Effects handlers
To understand this, we need to have an understanding of `@Compose` lifecycle. Composables join the composition when rendered on the screen and depart when they are removed from the UI tree. It might happen that effects will cross from one event to another. Effects usually have different lifespans. Some may have long lifespans while others have shorter lifespans. This allows you to utilize them in many compositions.
...
### SuspendedEffects
These are effects that may occur when we make long-running calls such as network calls inside a composable. Under the suspended effects, we have `rememberCoroutineScope` and `launchedEffect` techniques.

### LaunchedEffect
`LaunchedEffect` may be used to do tasks during the lifetime of a composable. The coroutine will self-destruct if the composable quits composition, or is no longer shown on the screen. This helps in avoiding memory leaks.

In general, the `lauchedEffect` does the following: 
- When an effect enters composition, the `launchedEffect` executes it and cancels it when it exits.
- When the key(s) changes, the effect is terminated and restarted.
- We can also use it to extend over a task in a composition.

Let's take a timer for example. We want this timer to start when composition of a composable begins and stop when the composition ends. We are going to do this regardless of the ending of the timer.
...
Because of these constraints, we prefer to use `rememberCoroutineScope` in such circumstances.

> NOTE: `rememberCoroutineScope`  generally returns a scope. This method produces a CoroutineScope that may be used to construct tasks that are children of the composition.
...
When the button is clicked, the coroutine is launched. This will start the timer in 2 seconds. A toast will appear indicating that the timer has started. After 2 seconds, another toast appears indicating that the time has ended.
...
In Non-Suspended effects, we can launch a side-effect to initialize a callback. This is done when the composition begins. After the composition is done, it is destroyed.
...
`DisposableEffect` is used to eliminate non-suspended effects. It commences when a composable begins composition and when the key of the composable updates.
...
We intend to attach the callback when the composable begins the composition, as well as when the dispatcher updates. The dispatcher is used as the effect handler key to accomplish this. In such a case, the effect will be disposed off and reintroduced.
...
### SideEffect
This is the simplest side-effect handler. With it, we have access to a block of code that is executed after every successful composition.
...
The best way to handle the side effect that may occur when you increment the variable `i` is by wrapping the code performing the incrementation in a side effect block as follows:
...
### Conclusion
In any app, we can encounter side-effects that we don't intend to handle in the `@Composable` body immediately. At some point, we may want it to be tied to the lifecycle of the Composable. We make sure that it executes in the correct lifecycle phase. We also ensure that it gets stopped to avoid memory leaks, and runs in a handy context `(CoroutineContext)`. This context is supplied by the effect handler when necessary by packaging it with the effective effect handler.
