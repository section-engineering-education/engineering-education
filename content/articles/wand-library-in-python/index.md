...
Using Python programming language, which created a library known as Imagemagick, then from here, wand library was generated. Using Wand library, images are manipulated in several ways which will be discussed later in the article. We can use the Wand to edit and open images, but in collaboration with NumPy, it offers significant functions for machine learning codes. Below are some of its uses.
- We can use it to display a single image in several outlooks eg upside down, blurred, etc.
...
Again, we must include the ImageMagick dependencies because the wand library is an Imagick API. The reason for including the dependencies is that they contain the functionalities of manipulating images e.g drawing.
```python
...
Through the implementation of the Wand's picture library's Image module, one will import image files. After that, We will use numerous properties such as height, width, and so to interpret the image. 

Now let us look at a complete example of checking the width and height of an image by following the above three steps.
```Python
from Wand.image import image as wandImage  # We will start by importing the image library from the Wand.
...
```
- The initial step importing the image library from the Wand.
...
- We shall then call the blur function by specifying the radius and sigma of the image. The radius parameter is used to determine the radius of the gaussian aperture, which is the size of the gap, and it is always an integer number, e.g., 3. Gaussian aperture function is to focus on what will be output. The sigma parameter is used to describe the value of sigma, and it is also an integer input, e.g., 5.
...
Another essential and robust feature of the Python wand is the image transform feature. It is used to transform an image to be displayed differently from the display of the original image for e.g an image can be transformed and be outputted as an upside-down image. 

Image transformation, in this case, means that the output of the image will be upsidedown and reversed as compared to the input image. The image transformation is achieved through the collaboration of several functions. 

Let us consider a complete example.
```Python
from wand.image import Image  # Let us import image library
...
- The initial step in the description example above is to import the necessary modules that are required i.e color, image, and drawing so that the desired output may be found. 
- After that specify the color of the image and the size in which the image shall cover.
...
Apart from the wand library that we have discussed, other libraries are used to manipulate images i.e pillow and opencv. We are going to look at how these libraries differ from each other.

OpenCV library is developed using C and C++ programming languages while the pillow is developed using C and python programming languages. However, OpenCV is faster as compared to pillow when dealing with image manipulation. In the same manner, OpenCV still becomes superior to wand library and hence it becomes the best out of all three that can best manipulate images.
### Conclusion
