...
To keep up with the rapidly changing tech world, we'll learn how to develop an Augmented Reality App in Kotlin in this article.
This app will allow you to use your phone's camera to add characteristics to an image.

### Prerequisites
- You should have installed [Android Studio](https://developer.android.com/studio) V4.0+.
...
It is a new technology that merges digital elements with real-world objects. Mobile phone users may interact with their environments using their smartphones.
With AR, we can bend reality in how we want, which is like an extension of reality, or, we can say, re-evaluating the future. 

### Creating a project
To create a new Android Studio project:
...
```
Also, in the `<application>` body, add the following:
```xml
<application ..>
    ...
            <meta-data
...
```
An error may appear on line
```xml
android:name="com.google.ar.sceneform.ux.ArFragment"
```
To solve this, open `build.gradle` app level and add the following dependency:
```gradle
implementation 'com.google.ar.sceneform.ux:sceneform-ux:1.17.1'
```
Sync the project to resolve the issue. 

### Implementing Apps main logic
Open `ActivityMain.kt` and first create a function that checks if a device supports ARcore. The function should be as follows:
```kotlin
private const val MIN_OPENGL_VERSION = 3.0 //Global variable
...
private fun isDeviceArSupported(context : Context) : Boolean {
    when {
        Build.VERSION.SDK_INT >= Build.VERSION_CODES.N -> {
            ...
            Toast.makeText(
                this,
                "Android version should be 7 or later versions",
                Toast.LENGTH_LONG
            )
                .show()
            this.finish()
...
The next thing will be adding our model to a scene. This will be done by creating a function that will place a node into an AR scene. The function is as folows:
```kotlin
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isDeviceArSupported(this)) {
            ...
            arFragment = (supportFragmentManager.findFragmentById(R.id.sceneform_ar_scene_view) as ArFragment?)!!
