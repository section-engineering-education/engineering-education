...
A quantum computer of `n` qubits can be in a combination of `2^n states`. For example, for one qubit, you have a probability distribution over 2 states. For two qubits, you have a probability distribution over 4 states and so on. This is the core difference between classical and quantum computers. Classical computers can be in any state you want, but, one at a time. Quantum computers can be in a superposition of all those states at the same time.
...
To install qiskit, you have two options. You can install it locally on your local computer or on the Jupyter Notebook hosted by IBM. To install locally on a Linux machine, write the following code:
