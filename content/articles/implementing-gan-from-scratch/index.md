...
description: We learn the basics of Generative Adversarial Networks and implement it from scratch in PyTorch. 
...
Before beginning the discussion about GANs, it is essential to understand generative modelling as it is the foundation of the concept of a GAN.
...
To explain this mathematically, let's assume that we have a dataset with $x$ instances and corresponding $y$ labels. There are mainly two types of modelling in Machine Learning:
...
- Take a random data sample $x$ from our training dataset, labelled as **real data**.
- Again, take a fake data sample $x'$ generated by the Generator network, labelled as **fake data**.
Both of the inputs are passed through the Discriminator neural network, which computes the classification errors.
Finally, backpropagate the total error to update the weights and biases of the Discriminator network, seeking to **minimize** the classification errors.
...
import numpy as np
```
If you have a GPU present in your machine, the device will automatically be selected as **Cuda**; otherwise, it will be **CPU**. Also, we are setting up the hyperparameters required for training in the cell below.
...
In the cell below, we have defined our Generator class which inherits from **nn.Module** class. To keep it simple for now, we have defined our Generator model as a neural net consisting of two hidden layers only. First, the dimensions of the random noise $z$ and image are passed as arguments to the constructor of our class. Then, it takes the random noise as an input to the network.
...
We don't have to download the MNIST dataset explicitly, PyTorch library's **datasets** class will download and transform the MNIST dataset for us. In transformation, we need to normalize the images before feeding them to the neural network and converting them into a tensor. Finally, we will load data to feed to the neural net using the **DataLoader** class.
...
As we have discussed the [training of GANs](#training-a-gan) earlier, the same principle is implemented programmatically with the help of the PyTorch library. Notice we are backpropagating **lossD**, which is the average of **lossD_real** and **lossD_fake** to update the weights and biases of the Discriminator network.
...
We have implemented a GAN in this blog, generating new data based on the MNIST dataset using random noise. A point to be noted is that we had to deal with MNIST data here whose distribution is quite simple to be modelled by the Generator, but what if our data is highly complex and has multi-model distribution, this is still an issue with the GANs and it restricts the ability of GANs to produce diverse results.
