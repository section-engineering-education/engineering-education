...
2. In the `domain` package created above, create a file named `Customer` and add the code below to it.
...
The above class has an `id` field for the primary key in the database, `lastName` and `firstName` fields that we will get from the [data.csv](https://replit.com/@okeloviolet/SpringBatchProcessing#SpringBatch/src/main/resources/data.csv) file.

### Repository layer
1. Create a new package named `repositories` in the root project package.
2. In the `repositories` package created above, create an interface named `CustomerRepository` and add the code below.
...
2.  In the `processor` package, create a new Java file named `CustomerProcessor` then add the code below.
...
```
The above class transforms data from one form to another. The `ItemProcessor<I, O>` takes in the input data (`I`), transforms it, and returns the result as the output data (`O`). In our case, we have declared the `Customer` entity as both the input and output, meaning our data form is maintained.


### Configuration layer
1. Create a new package named `config` in the root project package. This package will contain all of our configurations.
2. In the `config` package, create a new Java file named `BatchConfiguration` and add the code below.
...
3. In the `config` package, create another Java class named `JobCompletionNotificationListener` and add the code below.
...
In the resource directory, add the code below to the `application.properties` file.
