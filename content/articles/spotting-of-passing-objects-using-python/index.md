...
Video can be said as a series of continuous images with the difference in time. So, we will have to initially identify and recognize the frames then have them processed.
...
#### Blur the frame
`GaussianBlur` from `cv2` is responsible to blur the image. It smoothens the pixels and averages out the intensity across pixel height against its width, thus filtering high-intensity noise.
...
- We find that the first and second frames already converted to grayscale. 
