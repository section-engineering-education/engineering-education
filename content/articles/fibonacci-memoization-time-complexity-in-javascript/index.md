...
date: 2022-02-11T01:00:00-17:40
topics: [Security]
...
<!--more-->
In software development, time complexity describes the amount of time taken by the computer to run an algorithm.
...
This is because the time taken to compute `fib(n)` equals the quantity of time we will take to compute `fib(n-1)` and `fib(n-2)`. We should also include constant time in the addition.
...
We add the previous two numbers in the sequence, e.g 5+8=13 to generate the next number of the sequence. The number needs to take a position of the sequence e.g 13 is the 7th Fibonacci number.
...
The above code displays the expected results. Let's now allocate a bigger number to the `fib()` function. We will determine the *70th* Fibonacci number as follows:
...
We will use a recursive tree to visualize our problem. Let's trace through what happens when we call the `fib` method with the number 7. 
...
In our tree, we will have one and two as our base cases. They will return 1 to the parent. We then add these two values to find the next number in the sequence. 
...
#### Time complexity
Classic recursive implementation usually has `0(2^n)` time complexity as shown above. To find the time complexity of our algorithm, we should consider how many times we will be calling `fib()`.
...
We have two additional nodes each time our node branches off, so it's `2*2*2... = 2^n.` making it `O(2^n)`. Note that it's not usually `2^n`. 
...
### Memoization
`0(2^n)` is not a desirable time complexity. This means if we ask for fib 70 it will take `2^70` steps to be executed. Therefore, we will get a large number because it takes a lot of steps. 

Let's search for any repeated sub-trees in the recursive nature of our tree. The subtrees can duplicate themselves.

To implement memoization, we need to capture a duplicate subtree, reuse these calculations then store them. This information may be useful when we will need to recalculate certain details.
...
> `(n in the memo)` condition is just some classic JavaScript syntax. We are checking if some key is inside the JavaScript object.

Let's store our entire result inside of our memo, our key is `n`. We will complete our initial OPERATION and return what we have put in that memo. Therefore, we will not change any return values. 
...
The next step is to ensure that all recursive function calls are accessing the same memo. We should pass a memo object to all of the recursive calls.
...
In the above code changes, we added a new argument and a base case.
...
In the above image, the red nodes return the memoized result. If they are ignored, the algorithm is called once for each value from *0 to n*.

A call tree for a figure larger than `fib(7)` will look just like the tree in the above image.
