...
### Why is the multi-module architecture better than monolith?
...
In multi-module, each service is compartmentalised before being placed in the big box; the project.

Multi-module allows for code re-usability unlike monolith. Code maintenance and adding new features or functionality is much easier.

#### Advantages of multi-module over microservices
In microservices architecture, services are built separately as individual projects and deployed separately.

For instance, the authentication service for a bank, built for just authenticating users and requests are deployed to an AWS server instance that communicates over some structured means with another service. Say "Transfer Service" of the same bank, built for transfers but deployed elsewhere, like Heroku.
...
Searching this out to be fixed can be challenging, especially if the service reporting the downtime is not a bogus, popular service within the organisation.

In multi-module, there is only one project, but several compartments or modules, each representing a service – authentication module, transfer module, etc., all housed in that project.

This project is hosted in just one place and so, its server’s availability means a total system availability every time. This approach also lowers the cost of hosting projects; microservices is clearly more expensive to maintain.
...
To build this, we would need some services, say `Doctor Service`, `Patient Service`, `Consultation Service`.
...
If you have an project already open, click `File > New > Project` and if this is your first time, click `New Project`.
...
First the data module. This module houses the entities for the project and the interfaces (JPA Repositories) that allow us interact seamlessly with out database, MySQL in this case.
...
As mentioned earlier, the data module holds the models and database operations for the entities, the services houses the implementations of the entities and the controller is the entry point into the project, where the endpoints are created.
...
Now, `service` can use all the methods in `data` easily. You can also pass the `service` module into the `controller` module this same way.
...
You basically know how to create a multi-module project now.

If the hospital needed to send mails to patients and doctors, they would need a notification service.
...
- Open the project, right click on the project name; `hospital-project`, in this case, click on `project` and then `module`.
...
- You can try to create a `Logistics Service` to represent the logistic department of the hospital.
...
All you need to do is to write `<packagin>jar</packaging>` in the POM file of the module you want to resuse as jar – ideally, in all the modules.

Spring Boot automatically does the rest for you. Now you can reuse that modules anywhere else.
...
- We looked at the merits and demerits of the multi-module architecture with respect to both monolith and microservices architecture.
- By using a project, we were able to setup a small multi-module project, set up modules and passed some dependencies around.
- The bonus section required that reader of the tutorial created a new module, using the approaches described in the tutorial.
