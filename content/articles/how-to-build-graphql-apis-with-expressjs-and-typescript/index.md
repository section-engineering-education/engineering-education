...
images:
  ...
  - url: /engineering-education/how-to-build-graphql-apis-with-expressjs-and-typescript/hero.jpg
...
![gql](lpwwbywliwnnakwtdica.png)

GraphQL is an API specification similar to the REST and SOAP specification. But in GraphQL, all requests have the same HTTP method, POST, and a single URL endpoint. You can check their official docs [here](https://graphql.org/). Now that you have an idea of GraphQL, you might be thinking, why so much noise about it? Why can’t we just stick to REST? The following reasons don't mean that the REST or SOAP specifications are obsolete, they only highlight areas in which GraphQL shines, and this doesn’t mean REST will go, by the way.
...
2. GraphQL also allows us to ask for exactly what we need. Let’s say we want to get all books created by a user in a typical REST API. We can access that by going to `api/v1/users/books/`, and we get an array of books. But in a situation where we just need the book title of each book, then the API is giving us more than we need or if, for example, we want to create a new book. We send a post request to `api/v1/users/books/` if everything should work fine; we do not want to start requesting for what we already have. Instead, we should just request things we do not have, like the `_id` of the book. But with GraphQL, we can query all books and get just the title of each book. We can do something like what we have below, and the server will return an array of books with just the title field in it.
...
GraphQL queries are used to perform read operations on the server, basically, things you will normally do with a GET request.

![gql queries](4l8v3uq95qquoy-4lyyj.png)
...
![gql mutations](qlvirbsc0jyjpj7lthxb.png)
...
![gql subscriptions](ykcmg34mni4xgkiv63b2.png)
...
![gql schema](q8c4tb4af_stv9gznqxo.png)
...
Firstly, we will create a book API where we can create books, update books, delete books, get all books, and get a single book. We’d need to create our project and set it up. Let’s name the api `bookrr`. Create a new npm project and give it the name bookrr. Your project structure should look like what we have below.

![project structure](b39mb_f-7rlshfezwmcj.png)
...
![book query](mm-hvr9wy-c11qahozb1.png)

![books and book query](n6tdf-szvwlksktstzqk.png)

Next, let us test our mutations.

![create and update book mutation](5ontsexvkbgaopsqfiep.png)
...
![delete book mutation](qy5g-zpmizfi4ci3pwfw.png)
...
This section deals with adding MongoDB and mongoose to our application. If you don’t have MongoDB locally, you can use a remote [MongoDB](https://www.mongodb.com/cloud/atlas/register) database. If you want to play around with MongoDB, you can try out MongoDB queries with test data on this [website](https://www.humongous.io/app/playground/mongodb).
...
Here we created the schema of our book model, and we export the book model.
...
Finally, we need to add the add the two exported objects in `db` folder to `db/index.js`
...
![books query db](taa-ha0eou-anlpluu_u.png)

![add books mutations](wg0le_lo7npttis20ya1.png)
...
![books and book queries](lkfygzg5ralkerckm32m.png)

Lastly, let’s test the `updateBook` and `deleteBook` mutations.

![update and delete book mutation](4wm6lynyaj4jgvflojgn.png)
...
 We need to convert all our `require` to `import` statements and all our `module.exports` to `export default`. The complete typescript source code can be seen [here](https://github.com/Babatunde13/bookrr). All I had to do to fix the errors was to add types and interfaces to the codebase, change all `require` to `import` statement, create interfaces(or types) for my database schema and other objects.
...
The way I structured the `schema` is suitable even for large applications; let’s take, for instance, an application with queries and mutations for `users`, `books`, `authors` etc. All we need to do is to destructure all the mutations and queries in the `resolver` object inside the `schema/index.js` file, but our `schema` will be in one file, which can be a pain in the ass as the app grows large. But we can even implement that easier with [apollo graphql](https://www.apollographql.com/docs/apollo-server/).
