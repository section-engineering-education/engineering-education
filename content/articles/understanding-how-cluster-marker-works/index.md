### Understanding how cluster markers work in web mapping using leaflet.js
  This is no more a news, as we are all aware that big data is the keyword of the 21st century, and It's shocking to realize that only a few people understand that a large part of data is categorized as geospatial. Nowadays, nearly all of the gadgets we use are satellite-based systems like GPS. Today, the simplest case of displaying data/marker on a map with an acceptable user experience may be a real challenge especially for the user as they might find it difficult to understand and to analyze data at a particular zoom level. For instance, a platform that has users with hundreds of markers on the map, when you zoom out the map, these markers all overlap and make the map appear messy and crowdy. To improve this kind of map, it is recommended to use the marker clustering method.
...
### Goal
At the end of this guide, we'll build a map with clustering features, and these include:
...
Changes won't effect yet on your browser, because we haven't initialized our map. To initialize it, set the map-view to Africa using Google Street Map. Create a script tag, copy and paste the snippet below to the script tag.
...
### Adding Data To Map
Any kind of data can be added to the map. but for the sake of this tutorial, we'll use the data generated from [geojson](geojson.io). Below is the image of the data generated from [geojson](geojson.io).

![image info](engineering-education/understanding-how-cluster-marker-works/data.png)

Save and download the file generated as a GeoJson file. Create a file and name it `mapdata.js` Inside the `mapdata.js` create a variable `map_data` setting the value to your downloaded generated data. To create the variable, copy and paste the snippet below.
...
Go to the file you just downloaded and copy all the data into the `mapdata.js` variable that is initialized
...
If you check the browser, you will notice that there are no changes and to render the data on the map you have to load the geojson data using the snippets below:
...
Save and check changes, all your markers should be represented like this:
...
### Clustering Marker
Whenever you are working on a project that has to do with a map, in which the map will be having multiple pointers, it's more efficient to use of marker clustering method.
...
### Conclusion
As we all know, in analyzing big data, you need a sophisticated tool like a marker cluster, which has been very helpful when it comes to mapping-related projects. You can view the source code for this project [here](https://github.com/Adebogunabdulroheem/Cluster-Markers).
