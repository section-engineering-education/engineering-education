...
### How to Get Started
Have you ever attempted to use a map to visualize a big quantity of data? If you use the traditional way, you'll wind up with something like this, with a lot of markers strewn about.
...
```
After you've added the 'OSM,' you'll see an output that looks like the image below, with the zoom level we set to the map with the 'longitude' and 'latitude' remaining intact.
...
(engineering-education/understanding-how-cluster-marker-works/map-marker.png)![image info](engineering-education/understanding-how-cluster-marker-works/map-marker.png)

## Styling Using Markers
Each marker should be represented by a circle in this case.
Copy and paste the snippet below into the code above the `geojson loader` to achieve this.
...
This script should be placed inside the body tag. On the browser, adding all of the CSS and JS links above has no effect.
