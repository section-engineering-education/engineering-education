...
This next step involves getting some historical data and preprocessing them. This process is essential as the data isn't in an appropriate format to perform calculations. Let's take a look at the current data types:
...
dtype: object
```
As we've seen, the data is stored as objects. We'll need to convert this data to numerical so that we can perform calculations on them.
...
If we type `hist_df.dtypes`, we'll notice that more than half of the columns are stored as objects (string). Since we are working with financial data and we would want to do computations, we need to convert this into numerical values. 
...
dtype: object
```
We begin by converting the `Open Time` and `Close Time` to date-time values. To do this, we'll use the `to_datetime()` function from pandas. As dates are returned from Binance as timestamps, we first divide by 1000 and then set the units to seconds to convert correctly.
...
```
We will visualize the `Close Time` data and limit the data to only the last 100 rows of data. In the plot, we've also included the candlestick type of plotting, volume data, the title for our plot, and finally, the 10, 20, and 30 moving day averages.
