...
author: ali-benard
date: 2022-02-03T00:00:00-17:00
topics: [Security]
...
### Introduction
MongoDB is one of the most used modern web applications for a database. MongoDB is not secure by default. Once installed, one will have a difficult time before configuring it for authentication. Because people can browse, write, delete, or modify data on your server without ever needing to log in or authenticate. Securing the database is simple and may be completed in a few steps. In this tutorial, you will learn how to secure and activate authentication on MongoDB.
...
ps -e|grep 'Mongod'
```
If there comes MongoDB is already installed in your machine, the output will be similar to this:
...
```
If MongoDB is installed on the same machine where your web app is installed, there may be some communication between the database with the application. To end this kind of linkage set the Ip address to 127.0.0.1. This will automatically ends communication via the internet.
...
#getlastError can be optional, writes and checks if the user exists.
```
