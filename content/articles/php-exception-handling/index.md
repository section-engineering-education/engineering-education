...
**try**
This is the block where the code that is likely to have an exception is typed. During runtime, if the code has exceptions, the exception is caught and corrected in the catch block. The try block is usually used with the finally or the catch block. A try block is usually used together with one or many catch blocks in a program.
