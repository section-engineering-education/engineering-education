...
The main window and several dialog boxes are common features of graphical user interface (GUI) apps. Use Qt Designer to quickly and design these graphical components. 
...
When installing Qt Designer, it all comes down to what kind of platform you're running and how you want to get it. Run the following commands from the terminal on Windows or Linux:
...
To use Qt Designer, you must first install it using one of the ways mentioned above. After the installation, you will see two windows:
...
After starting Qt Designer, a New Form dialog will display. This dialog box has five distinct GUI templates. Everything you need to get started is included in the package. Widget, Main Window, Dialog with Buttons Bottom, Dialog without Buttons, and Dialog with Buttons Right are examples of these templates.
...
Use the New Form dialog box templates to get started, then click Create to finish.

Choose a Qt Designer template from the New Form menu, and press Create or Alt+R to build a new form.
...
For input devices, Box provides a Scratchpad at the bottom of the window. This category has a part dedicated to valuable items that may be sorted by kind or function. You may drag and drop widgets from the Widget Box onto the Scratchpad. For removal, right-click the Scratchpad widget and select Remove.

Object Inspector navigation by the tree. Object Inspector's search filter box lets you discover objects quickly. It can edit the form's name and other components. Right-clicking a widget offers the following options:
...
### Build the Main Menu
The Main Window template of Qt Designer may be modified. The Menu Editor may extend the main menu bar. In applications, drop-down menus are usually lists of alternatives. Open Qt Designer and the new window you created. Type Start typing in the form's menu bar.
...
A menu choice may have a submenu. Choose Open Recent from the menu and then click Create Submenu. This adds a submenu to the Recent Open menu.
...
The use of keyboard shortcuts will make things simpler for the user.
...
To reuse actions generated when filling menus, drag & drop them into the toolbar. Add actions to the toolbar by dragging them from the Action Editor. Right-click the toolbar and select Add Separators from the pop-up menu.
...
Use it as the main widget on the GUI for the main window to stack one or more QWidget objects.
...
Your primary window's core widget may be a collection of widgets in a specific arrangement. Please create your widget arrangement and use it as the template's top-level layout, with a QWidget object as its center widget.
...
- To organize the widgets on your form, drag & drop them from one location to another.
