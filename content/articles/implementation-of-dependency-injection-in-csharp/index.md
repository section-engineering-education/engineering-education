...
Dependency injection makes unit testing- testing single units of the source code to ensure they are fit for use- efficient and also improves code readability and reuse.
...
Dependency Injection allows one to write loosely coupled code -where two objects are independent of each other. This is achieved by separately creating the client's dependency on its behavior. Introducing dependency injection in the above example results in loosely coupled code- the inner class will not be dependent on the client class.
...
`public interface IClass` becomes the client class' dependency with the method serve() used to inject the dependency to class service1 and class service2.
...
Here we create objects of class service1 and service2, which are passed as parameters to object of the client class. The client class object c1 calls the serve() method that prints the string.
...
When only one method in a class needs the dependence, this method is suitable.
...
Setter Injection adds dependency on a client class's public property. No new object creation or update is required.
