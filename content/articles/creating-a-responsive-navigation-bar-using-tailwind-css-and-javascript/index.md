...
What is a navigation bar? A navigation bar is a tool at the top most position of a webpage. It is used by website visitors to steer through different sections in the website. When a webpage is long, a navigation bar comes in handy. It helps the user to easily navigate to specific sections of the webpage. A navigation bar should, therefore, be simple and easy to use.

In Tailwind CSS framework, you can easily style up your website by adding styles directly into your HTML file by using the various Tailwind CSS classes. This has made it much easier to create both mobile friendly and desktop friendly designs. The layouts of the webpage transform according to the size of the screen in which the website is being viewed on. This kind of website is said to be responsive.
...
Visit this [article](/engineering-education/introduction-to-tailwind-css/) for the complete installation process of Tailwind CSS framework.

After full installation of Tailwind CSS, we will link our Tailwind stylesheet using the link tag as shown:
...
This tag wraps the two `<div>` tags containing our website logo and our primary navbar items to make them as one division. We will add `flex` and `space-x-7` classes which will align our logo and the navbar items vertically and add some space between the items respectively.
...
- `font-semibold` class makes the text to appear semi-bold.
...
- `md:hidden` class hide the the mobile menu button when viewed in a medium screen size.
- `flex` class makes the hamburger menu will align side by side with the other items in the navbar.
...
### Conclusion
In this article, we have gone through creating our own resposive navigation bar using Tailwind CSS, we have also learnt how to create the toggle functionality for our navbar using JavaScript. A navigation bar is one example of the many things you create using Tailwind classes. A navigation bar is an important tool for user navigation experience.
