...
We'll see the theory of EHD, its implementation in Matlab and its effectiveness in image retrieval. Also, we interpreted the output for proper understanding.
...
_Shape descriptor_: They are region, contour, shape, 3D, and perceptual 3D.</br>
_Motion descriptor_: Include Camera motion, motion trajectory, parametric motion, and motion activity.</br>
...
> Note that the MPEG-7 EHD suggests only the 80points but, if we include the global bin, the results are much better than that of 80 points.
...
We use the `imshow()` function to show the image. Also, we create the bar plot of the global for both images using the `bar()` function. It is for easy comparison.
