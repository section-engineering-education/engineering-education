...
### Introduction
Wavelet-based denoising is a method of analysis that uses the time-frequency to select an appropriate frequency band based on the characteristics of the signal.

A signal describes various physical quantities over time. While, noise is an unwanted signal which interferes with the signal carrying the original message. This causes change in the parameters of the signal message. Denoising is the process of removing noise from the signal.

Wavelet analysis can be applicable in daily life activities such as feature extraction, face recognition, data analysis and prediction, voice recognition, numerical analysis and many more.
...
In this article, we will discuss about one such method called Wavelet-based denoising of images, which is one of the most efficient method.
...
The denoising scheme involves passing the signal through decomposer to be decomposed into various wavelet co-efficient using Discrete Wavelet Transform (DWT).
...
> Grayscale images are images that contain only a single colour with two possible intensity values, that is black and white. The values are often 0 for black and 1 for white.
...
Orthogonal wavelets is a single scaling function. When a wavelet is biorthogonal, it generates a single scaling function, a wavelet for decomposition and another for reconstruction. Scaling function can be used shrinking or magnifying the function.

Level decomposition involves passing signal through a low pass and high pass filter, thus making the resultant signal to be downsampled by 2.
...
`wdencmp` - It can be used for denoising of both 1-D and 2-D signals. This means it can be used for both audio as well as images.
...
The single thresholding method is used when the intensity distribution between the objects of foreground and background are very distinct. In level thresholding, thresholds are rescaled at each level to arrive at new corresponding to the standard deviation of wavelet co-efficient at each level.
...
We first load original image from the PC.
...
We read the image using `imread` function. The image `img` is the unnoisy original image, and so we add noise to make it noisy.
...
After defining the `thr`, `sorh`, `keepapp`, we pass them to the `wdencmp` function as shown below. This is to denoise images using image thresholding.
