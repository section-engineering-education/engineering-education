...
Once done, click on `automate`. Once clicked, the window displays the instruction for using the specified algorithm. Read through the instructions and click on the play button at the top of the window.
...
It is now up to the user to decide the algorithm to use depending on what he does. However, as we know that the accuracy of algorithms varies, not restricting to specific algorithms is a good idea.
