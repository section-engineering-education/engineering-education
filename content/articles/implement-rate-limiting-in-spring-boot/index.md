...
Rate limiters can also serve as a means to place consumers on payment plans, by controlling how many calls can be made with a plan they choose.
...
The idea behind this simple but powerful algorithm is straightforward.
...
Now, we will implement a rate limit using Bucket4j for a small application.
...
### Points to note
API calls are becoming more popular over the weeks. You must have used a service that required you to upgrade from one plan to a higher one – say, from a free tier to a basic plan.
