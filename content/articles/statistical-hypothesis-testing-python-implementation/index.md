...
4. Confidence of null hypothesis.
5. Statistical techniques for hypothesis testing.
6. Chi-square test - Python implementation.
7. T-Test - Python implementation.
8. ANOVA - Python implementation.
9. Conclusion.
10. Additional resources.

### 1. [Prerequisites](#1-prerequisites)
The reader of this tutorial should have basic knowledge of python programming, must have an idea of the concept of exploratory data analysis and how pandas and numpy work, must have elementary understanding of statistics, and must be a data science enthusiast.

### 2. [Goal of the Tutorial](#2-goal-of-the-tutorial)
...
### 3. [Understanding Hypothesis Testing](#3-understanding-hypothesis-testing)
There are two types of hypothesis testing, namely, null hypothesis and alternate hypothesis.

[**Null hypothesis**](https://en.wikipedia.org/wiki/Null_Hypothesis:_The_Journal_of_Unlikely_Science) is the initial assumption about an event (also referred to as the ground truth), whereas [**alternate hypothesis**](https://en.wikipedia.org/wiki/Alternative_hypothesis) is an assumption that counters the initial assumption". The null hypothesis is a statement believed to be true unless we can prove otherwise beyond a reasonable doubt.
...
### 4. [Confidence of Null Hypothesis (Probability Value)](#4-confidence-of-null-hypothesis)
...
P-value is within the range of 0 and 1, but the threshold value is always set at 0.05, the threshold value can also be called *alpha*, alpha is always set before the experiments in order to avoid bias. 
...
While inplementing statistical hypothesis testing, there are some methodologies that can be employed, in this tutorial, I will be touching on three methods of carrying out statistical hypothesis testing.

### 5. [Statistical Techniques for Hypothesis Testing](#5-statistical-techniques-for-hypothesis-testing)
The three methods to be discussed are:
1. Chi-Square Test
2. T-Test
3. ANOVA Test

### 6. [Chi-square test - Python implementation.](#6-chi-square-test-python-implementation)
...
A mathematics teacher conducted a test for a class of 20 students. He expected 5 to score good marks, 7 to score average marks while 8 to fail. Eventually, when the result came out, 5 failed, 6 were ln the average while 9 scored good marks. Mathematically, carry out chi-square test for this class.
...
chi-square = 4.468
```
For huge datasets, mathematical calculations cannot be done by hand. The implementation of chi-square test in Python is discussed in the next section.
...
```
![petalwidthdescribe](/engineering-education/statistical-hypothesis-testing-python-implementation/petalwidthdescribe.PNG)
...
```
![percentile](/engineering-education/statistical-hypothesis-testing-python-implementation/percentile.PNG)
...
print(dof)
```
![dof](/engineering-education/statistical-hypothesis-testing-python-implementation/DOF.PNG)
...
```
![alphsig](/engineering-education/statistical-hypothesis-testing-python-implementation/alphasiglevel.PNG)
...
### 7. [T-Test - Python implementation.](#7-T-Test-Python-implementation.)
...
the observed mean group be a.

the assumed population means be B.

the standard deviation of the data group be X.

the number of observations in the group be V.
...
Data sample A's mean be  Ma.

Data sample B's mean be Mb.

Size of sample be Na.

Size of sample B be Nb.
...
The difference between paired observations' sample mean be D.

assumed mean difference be m.

standard deviation be xd.

the number of observations in the group be N.
...
```
![onesampttest](/engineering-education/statistical-hypothesis-testing-python-implementation/onsamplettest.PNG)
...
```
![var](/engineering-education/statistical-hypothesis-testing-python-implementation/variancetwosamplettest.PNG)
```python
ratio_check = 0.6811222222222222 / 0.5785315555555559

ratio_check
```
![ratiocheck](/engineering-education/statistical-hypothesis-testing-python-implementation/ratiocheck.PNG)
...
```
![twosample](/engineering-education/statistical-hypothesis-testing-python-implementation/twosamplettest.PNG)
...
### 8. [ANOVA - Python implementation.](#8-anova-python-implementation)
...
A typical example: 
'The growth department of a certain company discovered an increase in sales for a particular product. In recent times, they've tried several advertisement channels so it is difficult to say which one yielded the sales increment the most.' 

They did online posters, billboards, TV ads, mouth-to-mouth, and sponsorship. By carrying out an ANOVA test on the several advertisement channels, they will be able to understand better which one works best out of all channels(independent variables), with the connection it has with the increase in sales which is the dependent variable. 
...
```
![1wayanova](/engineering-education/statistical-hypothesis-testing-python-implementation/onewayanova.PNG)
