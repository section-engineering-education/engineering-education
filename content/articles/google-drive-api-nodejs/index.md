...
Through the API, you can develop an application that seamlessly integrates Google Drive for data storage and sharing. Among the functionalities provided by the API are uploading, deleting, sharing, and downloading files that exist in a Google Drive account. 
...
To create the node application, we will run the command below to initialize a `package.json` file. 
...
We can obtain the  `CLIENT_ID` and `SECRECT` from the dashboard as we had created them earlier. 
In the `index.js`, add the snippets below to include the `client ID` and `secret`.
...
3. On the top right, click the `setting icon` then in the dropdown menu, check the `use your own Auth credentials checkbox. 
...
Now store the refresh token in constant as below.
```js
//redirect URL
...
We have set up everything for the project. Now we need to add the code snippets for the basic functionalities as specified in the project goal. We will write three functions to upload a file to google drive, delete a specified file, and generate a shareable link of a given file existing in Google Drive.

#### Upload file to Google Drive
The `uploadfile` function taken no parameter. It is an asynchronous function that sends a request and waits for a response from the server. In the `request body` we supply the `filename` and `mimeType`. The `mimeType` specifies the file format. You can find more about `mimeType` [here](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types). The snippets below shows the function to upload a file to google drive
```js
...
This function is responsible for creating a link to access the file in the drive folder. 
First the function should change the permission to public for anyone to access. This process is necessary because during upload, only the user who uploaded the file has access to the file.
```js
//create a public url
...
```
The `webContent` link downloads the file to the system directly.
The `webViewLink` is a link for viewing the file in a browser without any credentials.

### Conclusion
In this tutorial, I explained how to set up Google Drive with Node.js. We wrote functions to upload, delete and share a file in Google Drive. You can go ahead and try out the application on your computer by the source code [here](https://replit.com/@paulromans/Drive-api-node#index.js).
