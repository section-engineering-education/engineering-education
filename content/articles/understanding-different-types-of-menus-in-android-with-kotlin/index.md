### Introduction
Menus are fundamental user interface elements that are frequently used to store actions that would otherwise be buried. These hidden elements could be accessed by:
- Clicking a button.
...
- Have Android Studio installed on your computer.
...
![Options menu](/engineering-education/understanding-different-types-of-menus-in-android-with-kotlin/options-menu.png)
...
The `when` statement is used to evaluate the value of the `itemId`. If its value is equal to the value of the `R.id.<item_id>` variable, then the `Toast.makeText` method is called with the message indicating the clicked item. This indicates that the menu is working. We can therefore perform the desired action.
...
#### Floating contextual menus
When one long press (press and hold) on an item, this menu appears as a floating list of menu items. Support for floating contextual menu should be declared. This menu allows actions to be performed on one item at a time.
...
![Floating contextual menu](/engineering-education/understanding-different-types-of-menus-in-android-with-kotlin/floating-contextual.png)
...
![Action mode](/engineering-education/understanding-different-types-of-menus-in-android-with-kotlin/action-mode.png)
...
To inflate and handle the clicks of the popup menu, the following code is used.
...
![Pop up menu](/engineering-education/understanding-different-types-of-menus-in-android-with-kotlin/popup-menu.png)
...
![Checkable menu](/engineering-education/understanding-different-types-of-menus-in-android-with-kotlin/checkable.png)
...
#### Intent-Based menus
In contrast to intent-based menus, other menus are created using a menu resource file. Menus based on intent are dynamically added based on the application.
