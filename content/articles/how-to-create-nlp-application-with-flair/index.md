...
1. Have [python](https://www.python.org/) installed in your machine.
2. You must have a good knowledge of [python](https://www.python.org/).
3. You must a good understanding of machine learning modeling.
4. Working knowledge of [Pandas](https://numpy.org/).
...
```python
df = pd.read_csv("offensive and non_offensive_dataset.csv")
```
...
3. Dev/Validation set.
   This is the set of data that is used to optmize of our model perfomance.
...
Let's create a folder where we can store our three sets of data.
Since we are using [Google Colab](https://research.google.com/), the following command will create a folder named `data_fst` for us.
...
![Our folder structure is shown](/engineering-education/how-to-create-nlp-application-with-flair/data_model.png)
...
To start making predictions we have to specify which model we will be using. Lets load the our best model using `load()` method. We will use it to make predictions. This makes our model ready for use.
...
The first example, the output is offensive.
