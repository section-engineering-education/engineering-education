...
A data model is a set of data types that describe the data that is stored in a database. A good model allows you to describe the data in a way that is easy to use and understand thus making the database maintainable.

#### Firestore data model
In Firestore, data is stored in collections identified by a unique path. Each collection contains documents, which are key-value pairs. A document can contain any number of fields, including sub-documents and sub-collections.

### Implementing Firestore in Android
First, create a new project in Android Studio.

Head to the [Firebase console](https://console.firebase.google.com) and create a new firebase project. This will require you to have a Google account. Note that you must use the same account to sign in to Android Studio and the Firebase console.
...
After so doing, open the Firestore tab and click on the `create database` button. This will create a new Firestore database.

To start with, it is important to use the test rules to ensure that the database is working correctly before you deploy the project. This is done by clicking on the `test rules` button.

> Warning: Test rules allow anyone to access the database. This is not recommended for production use. Instead, you should use secure rules and authentication to restrict access to the database.

Finally in this step, we need to connect our app to Firebase. Click on the `Tools` tab in Android Studio, select `Firebase` then connect the projects by clicking the `Connect to Firebase` button. This will take you to the Firebase console where you need to select the project we just created.

Back to Android Studio, add `Firestore SDK` by clicking the `add cloud Firestore` button in the Firebase assistant.
...
In the `btnSaveData` button click listener lambda, call the above function, and pass the `crops` array in a coroutine.
...
You might have noticed a warning in the firebase console indicating that the `crops` document will not appear in queries since it doesn't exist. This is so because it only consist of collections. To fix this, add at least one document to it.
...
To get a specific document, we can use the `get()` method and pass the document id as a parameter. This is very useful when deleting or updating the fields but for you to know the id, it must be one of the fields in the document.
...
We can also create query objects that can be used to perform complex queries.
...
Indexes are used to make queries more efficient. Firestore automatically creates indexes as the data is saved. However, for custom queries, we need to create indexes for the fields involved.
...
The order of the conditions/filter operations is very important as the result of the first one is passed to the next one in the chain. Therefore, you should always start with less specific or the most relevant conditions.
