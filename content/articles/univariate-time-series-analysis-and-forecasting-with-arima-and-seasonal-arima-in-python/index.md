...
In time series, we have univariate, bivariate, and multivariate. Univariate consists of only one variable, bivariate we have two variables, and multivariate we have more than two variables. In this tutorial, we will be focusing on univariate time series modeling.

We will first discuss a few concepts that are important to understand time series with ARIMA and Seasonal ARIMA. We will finally implement the ARIMA and Seasonal ARIMA models with Python.
...
The information found in the past values indicates the forecasts of the future values. It assumes that the previous values have unique attributes and properties that can predict future values. For example, an ARIMA model can be used to predict future stock prices based on observing and analyzing previous stock prices.

The model analyzes the single time-dependent variable found in the times series dataset. ARIMA will model time series data based on its past values.

ARIMA only works when the time series data is stationary. When the dataset is non-stationary, we always have to change it to stationary before applying the ARIMA model.
...
When the dataset has trends, repetitive cycles, and seasonality effects, it becomes non-stationary. The trends and seasonality affect the properties and attributes of the data. They make them change with time.

Most time-series data is non-stationary, especially stock prices and other financial data. This is due to the trends and the huge swings available in the financial market. 

For ARIMA to use such time-series data, we have to make the data stationary. Making the data stationary will remove the trends and seasonality. It will be easier for ARIMA to make a prediction using stationary data. We change non-stationary data to stationary using the differencing technique. 
...
It transforms the variance, the covariance, and the mean values data by making them constant over time. It also reduces the repetitive cycles in the time series data.

Differencing is a method of transforming a non-stationary time series into a stationary one. This is an important step in preparing data to be used in an ARIMA model. The first differencing result is the difference between the current time and the previous time.

It finds the difference between the current data points and the previous data points to make the data stationary. We may difference once but still not produce a stationary dataset. In this case, we need to find the difference various times until the dataset becomes stationary.

### Components of ARIMA model
ARIMA model is made up of three components: Auto Regression, Integrated and Moving average. 
...
#### Moving Average
It uses errors of past values to make future predictions. It uses the dependency between an actual observation and a residual error from previous observation.
...
The lag is the time between two data points/observations in the time series. We determine the best value of `p` using a [Partial Autocorrelation Function](https://mxplus3.medium.com/interpreting-autocorrelation-partial-autocorrelation-plots-for-time-series-analysis-23f87b102c64) (PACF) plot.

- d: It is the total differencing steps performed to make the time series stationary. When we have time-series data that is already stationary, there is no need to conduct differencing. Thus, d=0.
...
There are many methods to check whether a time series is stationary such as direct observations and performing statistical tests.

#### Direct observations
It entails visualizing the time series plot to check for trends or seasonality. 

#### Performing statistical tests
It entails performing statistical tests on the time series to checks for stationarity. We use various statistical tools for this testing. In this tutorial, we will use Augmented Dickey-Fuller.
...
The ADF test has a parameter known as the `p-value` that determines if the time series is stationary or not. If the `p-value` of the ADF test is < 0.05, then the time series is stationary. So, if the p-value > 0.05, we will have to change the time series to stationary using differencing.
...
SARIMA is implemented using the ARIMA models parameters (p,d,q). We just have to add a few new parameters to handle the repeating cycles or seasonality in the data.

The newly added parameters are `P`, `Q`, `D`, and `s`.  SARIMA models are usually initialized as SARIMA(p,d,q)(P, D, Q, s). 
...
- `s`: It represents the periods in each seasonality component. The number of months in a year is 12. So,`s=12`

SARIMA handles seasonality using the `D` parameter. It performs seasonal differencing. Instead of subtracting the data points in the whole times series, seasonal differencing only subtract data points in the seasonality components.

Now that we have discussed the important concepts, let's start building our models.
...
We have dropped these rows, the new dataset will now have 104 rows.
...
We will plot time series data and check for trends or seasonality/repeating cycles. As mentioned earlier, this is a simple method to check for stationarity.
...
We will still need to perform a statistical test on the time series to prove this non-stationarity. As mentioned earlier, we will use the 
Augmented Dickey-Fuller test.
...
We will have to change the time series to stationary so that the ARIMA model can use. We use the differencing approach.
...
We will use this plot to get the best value of `q`. From the ACF plot, we can observe that lag no 1 stands out. It is slightly above (cuts off) the significance line (the blue shaded line). It is indicated using the red arrow. This is the number we will select to be the value of `q`.  Therefore, `q=1`.
...
We will use this plot to get the best value of `p`. From the ACF plot, we can observe that lags 1 and 13 stand out. They are above (cuts off) the significance line (the blue shaded line). They are indicated using the red arrow. 

We will select lag no 1 as the best value of `p`. This is the first lag that is above (cuts off) the significance line. Therefore, `p=1`. Our values will be: `p=1`, `d=1` and `q=1`

Let's import the ARIMA model.
...
We will train the ARIMA model using the `fit` function. 

```python
arima_model=model.fit()
```
We have trained the ARIMA model, we can use it to predict the test portion of the data. For prediction using the test portion, we will have the actual sales and the predicted (forecast) sales.

### Testing the ARIMA model using the test portion
We will use the values from the 90th row to the 103rd row. They will be our test portion/set. We will then use Matplotlib to show the actual sales and the predicted (forecast) sales. 
...
From the output above, the blue line is the actual sales and the orange line is the forecast sales. The ARIMA model has not performed well, it has not made correct predictions. The forecast line is far apart from the actual line. We will now build another model using SARIMA.
...
SARIMA models are usually initialized as SARIMA(p,d,q)(P, D, Q, s). We already have the `p`, `d`, and `q` values. We can get the `P`, `D`, `Q` the same way. Thus, p=P, q=Q, and d=D. `s=12` since there are 12 months in a year.
...
We will train the SARIMA model using the `fit` function. 

```python
sarima_model=model.fit()
```
We have trained the SARIMA model, we can use it to predict the test portion of the data. For prediction using the test portion, we will have the actual sales and the predicted (forecast) sales.
...
From the output above, the blue line is the actual sales and the orange line is the forecast sales. This is a good prediction, the two lines are close to each other.

### Conclusion
We have learned how to build a univariate time series model with ARIMA and SARIMA in python. We discussed how the ARIMA and SARIMA models work. We also talked about time-series stationarity and how to make the series stationary using differencing. We covered how to check for stationarity using the Augmented Dickey-Fuller test.

We explored all the components of the ARIMA model and their specific parameters. We used the autocorrelation Function (ACF) plot and Partial Autocorrelation Function (PACF) plot to get the best `p` and `q` values
