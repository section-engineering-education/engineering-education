...
After being conversant with creating your APIs using the Django REST framework, you may find that you need to alter or create data on your django apis using data from other django apis.Some form of communication is therefore required to facilitate these operations.

We are going to use RabbitMQ to enable this communications.RabbitMQ is an  messaging service that allows microservices to communicate with one another.It offers the following advantages:

1. Order and delivery guarantee.
2. Redundancy
3. Scalability

RabbitMQ works by having publishers/producers creating messages and send them to an exchange with a `routing-key`.Consumers then call and receive the messages and process them accordingly.

### Prerequisites.
For you to follow along, it's important that:
1. You are able to create a Django application and have basic understanding on how build APIs using the [Django REST framework](https://www.django-rest-framework.org/).
...
### Takeaways.
1. Have an understanding on how RabbitMQ works.
2. Communicating between your APIs using RabbitMQ as a messaging service.

### Getting Started.
Consider a scenario where an API contains details about different quotes and their associated number of likes. We also have another API that handles the liking of a quote such that a user can only like a qoute once and when he/she does the number of qoutes are incremented.
...
'corsheaders'
```
You need to have Django rest framework and django-cors-headers installed.You can run `pip3 install djangorestframework` and `pip3 install django-cors-headers` to install them.
...
We can proceed to creating our model. Modify your `models.py` file to look as this:
...
class UserSerializer(serializerms.ModelSerializer):
    class Meta:
        ...
        exclude = ('password', )
```
Let's create some views in order to render data to a web page.There happens to be several ways for creating views and our `views.py` file will have two for a start.
...
### Likes Project.
Just like the `Quotes` project, lets create an app called `likes`. Cd into Likes and run `python3 manage.py startapp likes` and add the following to your `INSTALLED APPS` in your `settings.py` file.
...
```
I have decided to create a similar model class `Quote` to demonstrate how we can pass data using rabbitmq and create model instances using the data.We will also have an additional model class `QuoteUser`.We will create the application for now and see how the communication and processing occurs in a short while.

Let's now create a `serializer.py` file that handles serialization of our model instances. Edit the file to the following lines of code:
...
need to make our Quotes application a producer and the Likes project a consumer. In order to get started, lets create a `producer.py` file in the `Quotes` project inside the `quotes` app.
...
1. These following two lines are responsible for establishing a connection with the RabbitMQ server.
`connection = pika.BlockingConnection(pika.ConnectionParameters('localhost', heartbeat=600, blocked_connection_timeout=300))`
...
The `method` parameter is the information regarding a message and `body` is the message to be sent.
...
In order to publish messages, we will use the `publish` function in our `views.py` file. Edit the file to looks as follows:
...
1. Similar to our producer application, we set a connection with the RabbitMQ server.

2. We also need to setup `DJANGO_SETTINGS_MODULE` because we are accessing a model while outside our `likes` app.
    ```python
...
4. We create a function `callback` that will be called whenever a message is received. The `ch` is the channel where communication occurs. `method` is the information concerning message delivery.`proprties` are user-defined properties on the message.`body` is the message received.
    In our case, the callback function handles creation, updation and deletion of `Quote` instances.The `property.content_type` are the properties we declared in our `producer.py` file.
...
`Editing a qoute at Quotes project`
...
We can now handle liking of a qoute. We will begin by creating a `producer.py` file in our `likes` app. It will be similar to the one in the `quotes` app and works the same way.
...
We would like a user to like a quote only once.We create a quoteuser when a quote is liked and the quoteuser can only be created once.When a user likes a quote, the number of likes is incremented by one on the Quotes project.
...
channel.close()
```
Just as with the previous consumer, we set up a connection and declared a queue called `quotes`.Our `callback` function gets a liked quote and increases its number of likes by 1.
...
![Liking a qoute at Likes](Liking_a_quote.png)

`http://127.0.0.1:8000/quotes/`
![Likes increased at Quotes](Quote_after_liked_at_Quotes.png)

Notice when one tries to like a qoute again by refreshing the page, a message is returned saying the quote has already been liked.

`http://127.0.0.1:1234/quotes/2/like`
![Liking a qoute more than once](Liking_a_quote_again.png)

### Conclusion
So far you have learnt the very basics of how RabbitMQ works and learnt how to use it to process data in your Django REST APIs. You should take a look on [RabbitMQ official site](https://www.rabbitmq.com/) to learn more.
