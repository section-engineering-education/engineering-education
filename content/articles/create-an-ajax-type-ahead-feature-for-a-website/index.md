...
Part of the job of a frontend developer is to create intuitive user interfaces to deliver the best user experience to website users. Part of getting this job done is creating search suggestions or what is commonly known as type ahead feature for websites/webpages.

In this tutorial, I will explain how to create an **ajax type-ahead feature** for your a website using [cities.json file](). This file contains informations about states and cities in America.

What this feature does is, if a user types `name`, every state and city that matches the word name will be displayed.
...
`Fetch`, like XMLHttpRequest, provides a method to get resources asynchronously across networks. However, the Fetch method provides an easy, cleaner and, more efficient way to handle resources across networks.
...
_You can create the Html however you want - what is crucial is that it has a search box and a container to display matching results._

- Create CSS to style as needed. In your CSS, add a `.highligh` or `.hl` style to format the search term when you return the result.
...
From the file, when get access to the following information of each state in America:
...
> The blobdata object, in turn, does not directly contain the actual JSON response body but is instead a representation of the entire HTTP response. So, to extract the JSON body content from the Response object, we use the json() method, which returns a second promise that resolves with the result of parsing the response body text as JSON.
...
_blobdata returns entire HTTP response_
    ...
    **Here are some things to note**
      ...
      - The empty cities array was declared with a const declaration, so we cannot reassign data into it.
...
- Set the inner Html of the ul element to the string returned from the above step.
  ...
  Left like this, the `card.innerHTML` will display every state and city when the search box is empty. To handle this, you can throw some control.
...
Next, lets format the output to highlight the text on the result which matches the text the user entered in the search box. Also, we will format the populatioin to be comma seperated.

This is achieve by replacing the following code.
...
In this tutorial, we have learnt how to build a type-ahead feature for a website. It display search suggestion containing words that matches the word a user type in the search box. We also learnt an optional way to format the output to highligh the matched words.
This tutorial can be implemented on any website, even with a different API or a different file.
