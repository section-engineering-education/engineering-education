...
This is an open-source javascript library that enables programmers to build beautiful user interfaces. You can use React.js to build those beautiful user interfaces on the front-end.
...
It is a browser extension that allows a user to access their Ethereum wallets on the Ethereum blockchain. Users can buy, store, and swap ether tokens using this library.
...
These two languages are used by the blockchain core developers to building the back-ends of blockchain systems.
...
It is also important to note that it is challenging to create a cryptocurrency. This is due to the vast amount of resources needed and distrust from potential users due to the reputation of Initial Coin Offerings (ICOs). The reputation of ICOs to investors has been dwindling recently due to lack of transparency of the companies that run these ICOs. ICOs have also been subjected to a large number of scams. Poorly informed investors have become victims to these scams which has led to them losing money. 

Lastly, as a developer, it is key to understand the difference between a token and a coin when it comes to cryptocurrencies. We use tokens and coins interchangeably when talking about cryptocurrencies. A cryptocurrency coin is classified as a store or exchange of value. For example, Bitcoin is a coin and not a token. You can use it as a store or exchange of value, like a currency. A token is a digital representation of an asset that can be exchanged using the blockchain network. Ethereum falls under this category with its ERC-20 tokens. The differences between the two is a bit cloudy but the difference is still there. Feel free to read this [blog post](https://blog.liquid.com/coin-vs-token) to understand the differences in depth.
...
These are a few use cases for blockchain technology. It can also be used to secure title deeds in real estate and to secure voting systems from tamper in democratic countries.
