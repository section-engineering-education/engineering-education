...
In order to follow through this article, a clear understanding of JАX-WS рrоtосоl is required.
...
The WSDL document infоrms a client's application regarding the many types of SIP messages that the web service sends and receives.

The WSDL file is a postcard that contains the location of a web service that can provide all of the functionality that the customer requires. In other words, the WSDL is similar to a postcard that contains the address of a certain site. The address gives the name and address of the person who hаndled the post for you.

Through out this tutorial, I will use Netbeans as the editor.
