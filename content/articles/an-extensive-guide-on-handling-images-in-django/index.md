...
### Goal
In this tutorial, we are going to build a Django application that handles images. By building, you shall learn how to set up your Django project environment to handle images and also how to perform other operations on the images as listed in the table of contents below.
...
- Have fundamental knowledge in both Python and Django
- Have a code editor.ie: Visual Studio code
...
Now we will need to install:
 - [Django](https://www.djangoproject.com/) - this python package helps build our website using the Django web development framework.
 - [Pillow](https://pypi.org/project/Pillow/) - this is a python image library that we shall use to manipulate our images.
...
```
The snippet we added above tells Django where to find user-uploaded images when the project is still in the development stage.

### Step 2 - Creating and registering the models
We can now proceed and define a model that shall be used for storing the images in our app.
In Django, a default database is automatically created for you. All you have to do is add the tables which are called models.
 In `models.py` we shall create a model with two fields, the title of a photo and the photo itself:
```py
...
```
The `list_display` list tells Django admin to display its contents in the admin dashboard. The contents are the model's fields and in this case, we want it to display the `title` and `photo` fields of every image that is uploaded.
...
#### The display view
Views in Django are used to send requests to and from the server. The requests can be returning a page, querying the database, making calculations,etc.
In `views.py`, add:
```py
...
```
- The above function-based view shall return all the Image objects from our database when requested.
...
```
Now we shall create another file named `urls.py` in our `demo` app directory. This new `urls.py` file is supposed to contain the url patterns of our app. Add the following code to it:
```py
from django.urls import path
from .import views
...
```
- `makemigrations` - generates the SQL commands for our new Images model.
Then also run:
```bash
...
```
- `createsuperuser` - Django comes with a ready-built admin site that can be used to administrate the users and the database of the site. When you run this command, you are prompted to enter credentials that you shall use to login into your account on that site.
...
```
- In the above snippet: upon the submission of a form, it will be checked for a POST request, and if it returns a positive result, the `ImageUploadForm` will be called to handle it. We then go ahead and check the validity of the form before saving it and redirecting the users to the display page where they can view it.
...
That would be the last part of this tutorial
### Conclusion
We have looked at various solutions regarding handling images in Django. I now hope you will be able to implement these solutions into your Django project as well.
You can also find the full source code [here](https://github.com/Donel254/Images)
