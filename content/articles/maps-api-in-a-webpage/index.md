...
<div class="col-xs-offset-2 col-xs-10">
   <button class="btn btn-success btn-lg " onclick="calculateDistance();"><i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i></button>
</div>
```
...
Create a new file called `app.js` in the same folder and the `index.html` file.
...
Google Maps covered the whole world that we cannot display on a single webpage. We need to choose a small Geographical location where our map will show.

My map covers the area around Nairobi, Kenya. I chose this location because that location is where I live, and I am familiar with the towns and cities around that area.
...
In our `index.html` file, we gave some `div` element and `id=googleMap`. Now to render the created map in the same `div` tag, add the snippets below in the `app.js` file:
...
Next, we need to render the direction on the same map object that we created before. We call the `DirectionsRenderer()` that will show the path on the map, then specify it to render it on our map using the `setMap(map)` method.
...
To add autocomplete of cities, we need to bring the inputs and then limit Google to show city names in a given country by specifying the country in the options object as below:
...
Lastly, we need to remove the dropdown of the list of cities after a given city is chosen. Use the snippets below:
...
If the direction is not available, the function eliminates the route from the map. The map gets re-centered to the current location and, the function returns an error message showing the user that it did not obtain the direction requested.
