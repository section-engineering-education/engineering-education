...
Active internet connection is not a guarantee of network connection capabilities. Knowing these capabilities will help you return the right information to the user.
...
We will then save these changes to `LiveData` objects. LiveData object will act as a data holder that can be observed by UI components. This makes it easy to keep what's showing on-screen in sync with the data saved in the LiveData objects. Let’s briefly discuss these concepts.
...
You need to track different states and lifecycle of an Android application to check different updates in your application components to show the user when connected and when the connection is lost.
...
This way, we only register a callback when needed and unregister a it when it's no longer needed. Such callbacks are removed from a request so that we don't waste any resources.
...
This creates a limitation since its shows are connected even to a network that has no internet. We are only detecting if there is an available network and if we are connected to that network.
...
- `networkCapabilities()` basically checks if this network is wifi enabled or cellular data. Technically there are two types of network. They could both have internet and can be passed to the `onAvailable()` function.
...
However, this connection is not valid, and it has `No Connection Capability`. Therefore, we cannot access any online data using this connection. And if you look at your application, the UI is updated accordingly with a `No Connection` TextView.
...
### Conclusion
This is a great use case to tell your users when there's network connection or not. You can use this in your app to pass the connectivity Boolean values and change your UI components relative to whether or not there's a network connection.
