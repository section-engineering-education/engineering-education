...
In order to solve the above issue, the categorical variables must be represented well and keep the relationship information between the items. Also, categorical variables have to be minimized to keep the items of the same categories packed together. The following section explains that this can be achieved by using embeddings.
...
#### PCA
[PCA](https://www.keboola.com/blog/pca-machine-learning) method is used to reduce the variables into small subsets to minimize the size of an entity. This method makes the variables complex to understand, which may cause data loss; however, it makes the model efficient.
