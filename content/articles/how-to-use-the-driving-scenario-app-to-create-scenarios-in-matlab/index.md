...
### Introduction
The Driving Scenario Designer App is used in designing synthetic driving scenarios. These scenarios are used in testing the automatic driving systems. The tool is mostly applied in developing automatic cars. It is because you can create all the scenarios using this toolbox. These applications include the creation of roads and adding actors into them. Actors are any mobile part of the road. One advantage of this application is that you add these scenarios using a drag and drop interface. Also, you can add radar and sensors to your ego car. The ego car is the car that you are trying to automate while considering the surrounding factors. These sensors can generate the line boundary and maintain your car within the boundary. 
...
vehicle1 = vehicle(driving,'ClassID',1','Position',[6 0 0],'Velocity',[-4 0 0],'Yaw',180)
```
...
```matlab
vehicle2 = vehicle(scenario,'ClassID',1,'Position',[0 10 0],'Velocity',[0 -7 0],'Yaw',-90)
```
The output is:
```Matlab
vehicle2 = 
    ...
    RCSElevationAngles: [-90 90]
         FrontOverhang: 0.9000
          RearOverhang: 1
             Wheelbase: 2.8000
```
After creating these scenarios programatically, we can visualize them by ploting them using the `plot()` function as shown below:
```matlab
plot(driving);
...
In adding roads, you specify the road centres and the bank angles. Bank angles are the turning points of the vehicle, and it is inclined at this point. When you plot these road centres and the bank angles, you get the road boundaries. To do this, we execute the code below:
```Matlab
driving = drivingScenario;
Centres = ...
...
```
In giving the trajectories, we specify the diameter of the car, the road offset and the rightway points and left way points. An offset line is a line a short distance from and parallel to a main survey line.
```matlab
diameter = 2.9/2;
