...
4. System developer. As a system developer, you can help integrate TFF with new types of development environments.
...
It's because the dataset in FL is obtained from multiple users. This poses a unique set of challenges that normal versions of the dataset don't exhibit. We import it into our project by using the `load_data()` function. 
...
```
In the training bit, you'll notice that we are only using a subset of client devices are selected to receive the training model. As much as we have many devices on the network, not all devices are eligible. At any given time, only a few devices may have relevant data to solve your problem. Thus, only a subset of devices is selected.
