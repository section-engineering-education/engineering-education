...
We first run the program to get the transfer function(Mc) and then add the step to get the step response. Basically, after doing this and re-running the program, we will have two plots, that is, the step response of the system without a controller and that of the system with the controller. Now, since the step response of the system with the controller has a proportional gain($$K_p$$) of 1, the plots will be the same.
![plot of the step responses](/engineering-education/pid-controllers/PID2.png)
*plot of the step response*

We will now look at the effects of changing P, I, and D because we did a step response but they are identical transfer functions. We will now change the PID controller constants. let's begin with $$K_p$$. This is done as follows;
