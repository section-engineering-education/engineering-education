...
Our application is now running successfully in the Kubernetes cluster but we cant access it from outside the cluster. To access our application from outside the cluster, we need to expose the service.
...
```
Now we can access our application from the URL [http://192.168.49.2:8080](http://192.168.49.2:8080) that is generated by minikube.
